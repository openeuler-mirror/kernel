...
retry:
	...
	nldsq_for_each_task(p, dsq) {
		 ...
		 * against scx_ops_bypass() dequeueing tasks from @dsq trying
		 * to put the system into the bypass mode. This can easily
		 * live-lock the machine. If aborting, exit from all non-bypass
		 * DSQs.
		 */
				 ...
				 struct affinity_context *ac)
{
	...
	if (SCX_HAS_OP(set_cpumask))
		SCX_CALL_OP_TASK(SCX_KF_REST, set_cpumask, task_rq(p),
...
void scx_cgroup_move_task(struct task_struct *p)
{
	...
	if (SCX_HAS_OP(cgroup_move) && !WARN_ON_ONCE(!p->scx.cgrp_moving_from))
		SCX_CALL_OP_TASK(SCX_KF_REST, cgroup_move, task_rq(p),
...
static bool scx_claim_exit(enum scx_exit_kind kind)
{
	 ...
	 * successfully reach scx_ops_bypass().
	 */
...
{
	struct scx_ops_enable_cmd *cmd =
		container_of(work, struct scx_ops_enable_cmd, work);
