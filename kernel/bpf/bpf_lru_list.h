...
/*
 * Mutually-exclusive per-CPU spinlock with re-entry rejection
 * (see implementation in bpf_lru_list.c).
 */
struct bpf_lru_spin_lock {
	raw_spinlock_t lock;
	int __percpu *nest;
};
...
struct bpf_lru_list {
	...
	struct bpf_lru_spin_lock lock ____cacheline_aligned_in_smp;
};
