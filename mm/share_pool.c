...
static int sp_mapping_group_setup(struct mm_struct *mm, struct sp_group *spg)
{
	...
	if (!list_empty(&spg->procs) && !(spg->flag & SPG_FLAG_NON_DVPP)) {
		/*
		 * Don't return an error when the mappings' address range
		 * conflict. As long as the mapping is unused, we can drop the
		 * empty mapping. This may change the address range for the task
		 * or group implicitly, give a warn for it.
		 */
...
/* The caller must hold sp_group_sem */
static int
sp_init_group_master_locked(struct task_struct *tsk, struct mm_struct *mm)
{
	int ret;
...
static struct spg_proc_stat *
sp_init_spg_proc_stat(struct sp_proc_stat *proc_stat, struct sp_group *spg)
{
	struct spg_proc_stat *stat;
	...
	struct sp_area *spa)
{
	...
	down_write(&sp_group_sem);
	stat = find_spg_proc_stat(tsk->mm->sp_group_master->stat, tsk->tgid,
				spa->spg->id);
	up_write(&sp_group_sem);
	if (!stat)
		return;
...
static int insert_spg_node(struct sp_group *spg, struct sp_group_node *node)
{
	...
	/*
	 * The only way where sp_init_spg_proc_stat got failed is that there is
	 * no memory for sp_spg_stat. We will avoid this failure when we put
	 * sp_spg_stat into sp_group_node later.
	 */
				     ...
				     pid_t applier)
{
	...
	if (!mapping->free_area_cache ||
	    size_align < mapping->cached_hole_size ||
	...
	if (mapping->free_area_cache) {
		first = rb_entry(mapping->free_area_cache, struct sp_area,
				 rb_node);
		addr = first->va_end;
...
static void trace_sp_alloc_finish(struct sp_alloc_context *ac, unsigned long va)
{
	...
	if (cost >= (unsigned long)sysctl_sp_perf_alloc) {
		pr_err("Task %s(%d/%d) sp_alloc returns 0x%lx consumes %luus, size is %luKB, "
		       ...
		       va, cost, byte2kb(ac->size), byte2kb(ac->size_aligned),
		       ac->sp_flags, is_local_group(ac->spg->id));
	}
}
...
unmap:
	sp_alloc_unmap(list_next_entry(spg_node, proc_node)->master->mm, spa,
			spg_node);
	return ret;
}
	...
	struct sp_group_node *spg_node, struct sp_alloc_context *ac)
{
	...
	if (ret) {
err:
		sp_alloc_unmap(list_next_entry(spg_node, proc_node)->master->mm,
				spa, spg_node);
...
static void *sp_make_share_kva_to_task(unsigned long kva, unsigned long size, unsigned long sp_flags)
{
	...
	stat = find_spg_proc_stat(current->mm->sp_group_master->stat,
				  current->tgid, spg->id);
	up_write(&sp_group_sem);
...
bool sp_config_dvpp_range(size_t start, size_t size, int device_id, int pid)
{
	...
	spm = spg->dvpp;
	default_start = MMAP_SHARE_POOL_16G_START +
