		...
		while (!kfifo_is_empty(&kfifo_error_task)) {
			...
			if (task_node->task_id == error_task_id) {
				if (task_node->handler) {
					pr_info("-----Measurement exception handler dispatching thread------\n");
					pr_info("Measurement exception received for task %d\n",
							error_task_id);
					pr_info("Step1: Query PSP for task %d status to confirm the error.\n",
							error_task_id);
					pr_info("Step2: Error confirmed, CALL measurement exception handler.\n");
					...
					pr_err("task %d's callback function is not registered, please check it\n",
						error_task_id);
				}
			}
		}
...
int psp_check_tdm_support(void)
{
	...
	if (is_vendor_hygon() && (ioread32(psp->io_regs +
					psp->vdata->feature_reg) & 1)) {
		if (tdm_support)
			goto end;
