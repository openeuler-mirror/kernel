...
static bool ne6x_can_reuse_rx_page(struct ne6x_ring *rx_ring, struct ne6x_rx_buf *rx_buffer)
{
	...
	if (unlikely((page_count(page) - pagecnt_bias) > 1))
		return false;
#else
#define NE6X_LAST_OFFSET (SKB_WITH_OVERHEAD(ne6x_rx_pg_size(rx_ring)) - rx_ring->rx_buf_len)
...
{
	if (ne6x_can_reuse_rx_page(rx_ring, rx_buffer)) {
