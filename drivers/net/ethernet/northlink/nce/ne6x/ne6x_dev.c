...
{
	return adpt->back->fw_info.fw_port[adpt->idx].hw_port;
}
...
{
	struct ne6x_fw_info *info = &adpt->back->fw_info;

	memset(mac, 0, 6);
	ether_addr_copy(mac, &info->fw_port[adpt->idx].mac[0]);
	return 0;
}

int ne6x_dev_get_port_num(struct ne6x_pf *pf)
{
	if (!pf->fw_info.nb_port)
		return 2;
	return pf->fw_info.nb_port;
}
...
{
	return ne6x_reg_set_vf_number(pf, num_vfs);
}
...
int ne6x_dev_add_unicast_for_spoofchk(struct ne6x_adapter *adpt, u8 *mac)
{
	struct spoofchk_table db;
	...
	return ne6x_reg_set_unicast_for_spoofchk(adpt->back, ADPT_VPORT(adpt),
						 (u32 *)&db, sizeof(db));
}
...
int ne6x_dev_del_unicast_for_spoofchk(struct ne6x_adapter *adpt)
{
	struct spoofchk_table db;
...
{
	struct ne6x_fw_port_info *fw_port = &adpt->back->fw_info.fw_port[adpt->idx];

	if (speed == SPEED_25000 && (fw_port->speed_supported & SPEED_SUPPORT_25G))
		return 0;
	if (speed == SPEED_10000 && (fw_port->speed_supported & SPEED_SUPPORT_10G))
		return 0;
	if (speed == SPEED_1000 && (fw_port->speed_supported & SPEED_SUPPORT_1G))
		return 0;
	if (speed == SPEED_100 && (fw_port->speed_supported & SPEED_SUPPORT_100M))
		return 0;
	if (speed == SPEED_40000 && (fw_port->speed_supported & SPEED_SUPPORT_40G))
		return 0;
	if (speed == SPEED_100000 && (fw_port->speed_supported & SPEED_SUPPORT_100G))
		return 0;
	return -EOPNOTSUPP;
}
...
int ne6x_dev_check_speed_change(struct ne6x_adapter *adpt)
{
	...
	if (fw_port->speed_supported & SPEED_SUPPORT_USER_CHG)
		return 0;
	return -EOPNOTSUPP;
}
...
int ne6x_dev_check_media(struct ne6x_adapter *adpt, u32 media)
{
	...
	if (media == PORT_FIBRE && (fw_port->media_supported & MEDIA_SUPPORT_FIBRE))
		return 0;
	if (media == PORT_TP && (fw_port->media_supported & MEDIA_SUPPORT_FIBRE))
		return 0;
	if (media == PORT_DA && (fw_port->media_supported & MEDIA_SUPPORT_DAC))
		return 0;
	return -EOPNOTSUPP;
}
...
int ne6x_dev_set_spoofchk(struct ne6x_adapter *adpt, bool setting)
{
	...
	ret = ne6x_reg_get_user_data(adpt->back, NP_USER_DATA_PORT_OLFLAGS_0 + ADPT_VPORT(adpt),
				     &val);
	if (unlikely(ret))
		return ret;

	if (setting)
		val |= NE6X_F_TX_SPOOF_CHECK;
	else
		val &= ~NE6X_F_TX_SPOOF_CHECK;

	ret = ne6x_reg_set_user_data(adpt->back, NP_USER_DATA_PORT_OLFLAGS_0 + ADPT_VPORT(adpt),
				     val);
	return ret;
}
...
int ne6x_dev_set_perm_mac(struct ne6x_pf *pf, u32 lport, u8 *mac)
{
	...
	indata[0] = lport;
	memcpy(&indata[1], mac, 6);

	return ne6x_reg_sub_cmd(pf, NE6X_SUB_CMD_SET_PERM_MAC, indata, 12, NULL, 0);
}
