...
static u32 ne6x_netlink_templ_read(struct ne6x_pf *pf, u32 *type)
{
	u32 value;
	u32 vport = 0;

	ne6x_reg_get_user_data(pf, vport + *(u32 *)type, &value);
	dev_info(&pf->pdev->dev, "temp_read  0x%04X value 0x%08X\n", *(u32 *)type, value);

	return value;
}

static int ne6x_netlink_templ_write(struct ne6x_pf *pf, struct ne6x_templ_write *templ)
{
	int err;
	u32 vport = 0;

	err = ne6x_reg_set_user_data(pf, vport + templ->port_num, templ->value);
	dev_info(&pf->pdev->dev, "temp_write: 0x%04x = 0x%x\n", templ->port_num, templ->value);

	return err;
}

static int ne6x_netlink_rcv_msg(struct nlmsghdr *nlh, u32 *result)
{
	...
	case NE6X_NLMSG_METER_WRITE:
		...
		break;
	case NE6X_NLMSG_TEMPL_WRITE:
		err = ne6x_netlink_templ_write(pf, data);
		break;
	case NE6X_NLMSG_TEMPL_READ:
		*result = ne6x_netlink_templ_read(pf, data);
		err = NE6X_READ_TYPE;
		break;
	default:
		return -EOPNOTSUPP;
...
static void ne6x_netlink_ack(struct sk_buff *in_skb, struct ne6x_netlink_status *data)
{
	struct sk_buff *skb_out;
	struct nlmsghdr *nlh;
	size_t payload;

	payload = sizeof(struct ne6x_netlink_status);
	...
	memcpy(nlmsg_data(nlh), data, sizeof(struct ne6x_netlink_status));
	nlmsg_unicast(in_skb->sk, skb_out, NETLINK_CB(in_skb).portid);
}

static void ne6x_netlink_rcv(struct sk_buff *skb)
{
	struct ne6x_netlink_status *netlink_status;
	u32 result = 0;
	u32 idx = 0;

	netlink_status = kzalloc(sizeof(*netlink_status), GFP_KERNEL);
	if (unlikely(!netlink_status))
		return;
	...
	while (skb->len >= nlmsg_total_size(0) && idx < NE6X_RULE_BATCH_MAX) {
		...
		if (unlikely(nlh->nlmsg_len < NLMSG_HDRLEN || skb->len < nlh->nlmsg_len)) {
			set_bit(idx, netlink_status->state);
			goto skip;
		}

		err = ne6x_netlink_rcv_msg(nlh, &result);
		if (err)
			set_bit(idx, netlink_status->state);
		netlink_status->data[idx] = result;
...
skip:
	...
	ne6x_netlink_ack(skb, netlink_status);
	mutex_unlock(&ne6x_msg_mutex);
	kfree(netlink_status);
}
