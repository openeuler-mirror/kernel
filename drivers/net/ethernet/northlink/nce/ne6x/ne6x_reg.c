...
#define AXIA_MBUS_COMMON_COMMAND          0x37
...
#define AXIA_MBUS_SET_VF_NUMBER_COMMAND             0xB5
#define AXIA_MBUS_SET_VF_NUMBER_ACK                 0xB6

#define AXIA_MBUS_SET_SPOOFCHK_COMMAND              0xD1
#define AXIA_MBUS_SET_SPOOFCHK_ACK                  0xD2
...
static int _ne6x_reg_perform(struct ne6x_pf *pf, u32 *data, u32 *pbuf, u32 len, u32 retlen)
{
	...
	if (test_bit(NE6X_PF_NIC_RESET, pf->state))
		return 0;
		     ...
		     u32 *outdata, u32 outlen)
{
	...
	msg = kzalloc((8 + inlen), GFP_KERNEL);
	if (!msg)
		return -ENOMEM;

	msg->hdr.uint = 0;
	msg->hdr.bits.opcode = AXIA_MBUS_COMMON_COMMAND;
	msg->hdr.bits.data_len = 8 + inlen;
	msg->data[0] = subcmd;
	if (inlen)
...
int ne6x_reg_get_sfp_eeprom(struct ne6x_pf *pf, int port, void *pbuf, u32 offset, int size)
{
	...
	status = ne6x_reg_perform(pf, (u32 *)msg, (u32 *)pbuf, 4, ret_size);
	kfree(msg);
...
int ne6x_reg_set_vf_number(struct ne6x_pf *pf, int num_vfs)
{
	...
	msg->hdr.uint = 0;
	msg->hdr.bits.opcode = AXIA_MBUS_SET_VF_NUMBER_COMMAND;
	msg->hdr.bits.data_len = 12;
	msg->data[0] = num_vfs;
...
int ne6x_reg_set_unicast_for_spoofchk(struct ne6x_pf *pf, u32 index, u32 *data,
				      u32 size)
{
	struct axia_mbus_msg *msg;
	int status;
	...
	msg->hdr.uint = 0;
	msg->hdr.bits.opcode = AXIA_MBUS_SET_SPOOFCHK_COMMAND;
	msg->hdr.bits.data_len = size + 8;
	msg->data[0] = index;
	memcpy((void *)&msg->data[1], (void *)data, size);
