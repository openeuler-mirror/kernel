...
static void ne6x_set_dflt_settings_vfs(struct ne6x_pf *pf)
{
	...
	ne6x_for_each_vf(pf, i) {
		...
		test_and_clear_bit(NE6X_VF_CONFIG_FLAG_SPOOFCHK, vf->vf_config_flag);
...
int ne6x_sriov_configure(struct pci_dev *pdev, int num_vfs)
{
	...
	ne6x_for_each_vf(pf, vf_id) {
		...
		if (adpt->hw_feature & NE6X_F_TX_SPOOF_CHECK)
			set_bit(NE6X_VF_CONFIG_FLAG_SPOOFCHK, vf->vf_config_flag);
	}

	err = ne6x_dev_set_vf_number(adpt->back, num_vfs);
	if (err)
		dev_warn(ne6x_pf_to_dev(pf), "Failed to set num_vfs: %d\n", err);
				    ...
				    struct virtchnl_ether_addr *vc_ether_addr)
{
	...
	ne6x_dev_add_unicast_for_spoofchk(adpt, mac_addr);
...
{
	ne6x_dev_del_unicast_for_spoofchk(adpt);
...
static int ne6x_vc_changed_rss_msg(struct ne6x_vf *vf, u8 *msg)
{
	...
	ret |= ne6x_dev_add_unicast_for_spoofchk(adpt, vf->dev_lan_addr.addr);
...
static int ne6x_vc_reset_vf_msg(struct ne6x_vf *vf, u8 *msg)
{
	...
	/* vf spoofchk status not change when vf remove */
	ne6x_dev_set_features(vf->adpt, vf->adpt->hw_feature & NE6X_F_TX_SPOOF_CHECK);
	ne6x_dev_del_vf_qinq(vf, 0, 0);
	...
	ne6x_dev_del_broadcast_leaf(ne6x_get_vf_adpt(vf));
	ne6x_adpt_clear_mac_vlan(ne6x_get_vf_adpt(vf));
	ne6x_dev_del_unicast_for_spoofchk(ne6x_get_vf_adpt(vf));
		       ...
		       struct ifla_vf_info *ivi)
{
	...
	if (test_bit(NE6X_VF_CONFIG_FLAG_SPOOFCHK, vf->vf_config_flag))
		ivi->spoofchk = 1;
	else
		ivi->spoofchk = 0;
...
static void ne6x_vc_notify_vf_spoofchk_change(struct ne6x_vf *vf)
{
	struct virtchnl_vf_config vfconfig = {0};
	struct ne6x_pf *pf = vf->pf;
	struct ne6x_hw *hw = &pf->hw;
	...
	dev = ne6x_pf_to_dev(pf);
	vfconfig.type = VIRTCHNL_VF_CONFIG_SPOOFCHK;
	if (test_bit(VIRTCHNL_VF_CONFIG_SPOOFCHK, vf->vf_config_flag))
		vfconfig.data[0] = 1;
	else
		vfconfig.data[0] = 0;
	...
	ne6x_sdk_send_msg_to_vf(hw, vf->vf_id, VIRTCHNL_OP_VF_CONFIG,
				VIRTCHNL_STATUS_SUCCESS, (u8 *)&vfconfig, 2);
}

int ne6x_set_vf_spoofchk(struct net_device *netdev, int vf_id, bool setting)
{
	struct ne6x_pf *pf = ne6x_netdev_to_pf(netdev);
	struct ne6x_adapter *adpt;
	struct ne6x_vf *vf;
	...
	spoofchk = adpt->hw_feature & NE6X_F_TX_SPOOF_CHECK;
	if (setting == spoofchk)
		return 0;

	ret = ne6x_dev_set_spoofchk(adpt, setting);
	if (unlikely(ret)) {
		netdev_err(netdev, "Set vf %d spoofchk %s failed\n",
	...
	if (setting) {
		set_bit(NE6X_VF_CONFIG_FLAG_SPOOFCHK, vf->vf_config_flag);
		adpt->hw_feature |= NE6X_F_TX_SPOOF_CHECK;
	} else {
		clear_bit(NE6X_VF_CONFIG_FLAG_SPOOFCHK, vf->vf_config_flag);
		adpt->hw_feature &= ~NE6X_F_TX_SPOOF_CHECK;
	}

	ne6x_vc_notify_vf_spoofchk_change(vf);

	netdev_info(netdev, "Set vf %d spoofchk %s success\n", vf_id, setting ? "on" : "off");
