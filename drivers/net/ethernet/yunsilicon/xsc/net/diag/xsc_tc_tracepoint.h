...
#if !defined(_XSC_TC_TP_) || defined(TRACE_HEADER_MULTI_READ)
#define _XSC_TC_TP_
...
DECLARE_EVENT_CLASS(xsc_flower_template,
...
DEFINE_EVENT(xsc_flower_template, xsc_configure_flower,
	     TP_PROTO(const struct flow_cls_offload *f),
	     TP_ARGS(f)
	     );

DEFINE_EVENT(xsc_flower_template, xsc_delete_flower,
	     TP_PROTO(const struct flow_cls_offload *f),
	     TP_ARGS(f)
	     );

TRACE_EVENT(xsc_stats_flower,
	    TP_PROTO(const struct flow_cls_offload *f),
	    TP_ARGS(f),
	    TP_STRUCT__entry(__field(void *, cookie)
			     __field(u64, bytes)
...
TRACE_EVENT(xsc_tc_update_neigh_used_value,
	    TP_PROTO(const struct xsc_neigh_hash_entry *nhe, bool neigh_used),
	    ...
	    TP_fast_assign(const struct xsc_neigh *mn = &nhe->x_neigh;
...
#define TRACE_INCLUDE_FILE xsc_tc_tracepoint
