...
static void hclge_dscp_to_prio_map_init(struct hclge_dev *hdev)
{
	...
	if (ret)
		dev_err(&hdev->pdev->dev, "failed to init dscp-prio map %d\n",
			ret);
}
...
static void hclge_tm_update_pf_tc_info(struct hclge_vport *vport,
...
static void hclge_tm_update_vf_tc_info(struct hclge_vport *vport,
				       u16 *vport_max_rss_size)
{
	struct hnae3_knic_private_info *kinfo = &vport->nic.kinfo;
	struct hclge_dev *hdev = vport->back;
...
static void hclge_tm_update_kinfo_rss_size(struct hclge_vport *vport)
{
	...
	if (vport->vport_id) {
		hclge_tm_update_vf_tc_info(vport, &vport_max_rss_size);
	} else {
		hclge_tm_update_pf_tc_info(vport, &vport_max_rss_size);
	}
