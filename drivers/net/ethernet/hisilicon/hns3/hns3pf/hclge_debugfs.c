...
static void hclge_dbg_dump_mac_frame_size(struct hclge_dev *hdev)
{
	...
	dev_info(&hdev->pdev->dev, "max_frame_size: 0x%x\n",
		 le16_to_cpu(req->max_frm_size));
	dev_info(&hdev->pdev->dev, "min_frame_size: 0x%x\n", req->min_frm_size);
}
		       ...
		       char *buf, int len)
{
	...
	if (strncmp(cmd_buf, HNAE3_DBG_TM_PRI,
