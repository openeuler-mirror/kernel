...
static void zxdh_dmaengine_dbg_summary_show(struct seq_file *s, struct dma_device *dma_dev)
{
...
static void __init zxdh_dmaengine_debugfs_init(void)
{
	...
	debugfs_create_file("summary", 0444, rootdir, NULL, &dmaengine_summary_fops);
}
#else
...
static ssize_t zxdh_memcpy_count_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct dma_chan *chan;
	...
	if (chan) {
		...
		err = scnprintf(buf, sizeof(buf), "%lu\n", count);
	} else {
		err = -ENODEV;
	}
	mutex_unlock(&dma_list_mutex);
...
static ssize_t zxdh_bytes_transferred_show(struct device *dev, struct device_attribute *attr,
					   char *buf)
{
	struct dma_chan *chan;
	unsigned long count = 0;
...
static ssize_t zxdh_in_use_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct dma_chan *chan;
	int err;
	...
	chan = zxdh_dev_to_dma_chan(dev);
	if (chan)
		err = scnprintf(buf, sizeof(buf), "%d\n", chan->client_count);
	else
		err = -ENODEV;
	mutex_unlock(&dma_list_mutex);
...
static int zxdh_dma_device_satisfies_mask(struct dma_device *device, const dma_cap_mask_t *want)
{
	dma_cap_mask_t has;
...
static struct dma_chan *zxdh_private_candidate(const dma_cap_mask_t *mask, struct dma_device *dev,
...
static struct dma_chan *zxdh_find_candidate(struct dma_device *device, const dma_cap_mask_t *mask,
					    dma_filter_fn fn, void *fn_param)
{
...
struct dma_chan *__zxdh__zxdh_dma_request_channel(const dma_cap_mask_t *mask, dma_filter_fn fn,
						  void *fn_param, struct device_node *np)
{
...
int zxdh_dma_async_device_channel_register(struct dma_device *device, struct dma_chan *chan)
{
	int rc;
...
void zxdh_dma_async_device_channel_unregister(struct dma_device *device, struct dma_chan *chan)
{
...
static int __init zxdh_dmaengine_init_unmap_pool(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(unmap_pool); i++) {
		...
		size = sizeof(struct dmaengine_unmap_data) + sizeof(dma_addr_t) * p->size;

		p->cache = kmem_cache_create(p->name, size, 0, SLAB_HWCACHE_ALIGN, NULL);
		if (!p->cache)
			break;
...
struct dmaengine_unmap_data *zxdh_dmaengine_get_unmap_data(struct device *dev, int nr, gfp_t flags)
{
...
static inline int zxdh_desc_check_and_set_metadata_mode(struct dma_async_tx_descriptor *desc,
...
int zxdh_dmaengine_desc_attach_metadata(struct dma_async_tx_descriptor *desc, void *data,
					size_t len)
{
	int ret;
...
int zxdh_dmaengine_desc_set_metadata_len(struct dma_async_tx_descriptor *desc, size_t payload_len)
{
	int ret;
