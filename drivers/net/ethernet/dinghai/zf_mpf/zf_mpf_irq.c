...
{
	struct dh_irq_range tmp = { .start = ZXDH_MPF_ASYNC_IRQ_NUM,
...
{
	struct dh_irq_range tmp = { .start = ZXDH_MPF_GDMA_MSIX_VEC_BASE,
...
static s32 irq_pools_init(struct dh_core_dev *dev)
{
	...
	mpf_irq_table->mpf_async_pool = (struct dh_irq_pool *)irq_pool_alloc(
		dev, irq_range.start, irq_range.size, "zxdh_mpf_msg", ZXDH_MPF_ASYNC_IRQ_MIN_COMP,
	...
	mpf_irq_table->mpf_comp_pool = (struct dh_irq_pool *)irq_pool_alloc(
		dev, irq_range.start, irq_range.size, "zxdh_mpf_comp", ZXDH_MPF_COMP_IRQ_MIN_COMP,
	...
	mpf_irq_table->mpf_gdma_pool = (struct dh_irq_pool *)irq_pool_alloc(
		dev, irq_range.start, irq_range.size, "zxdh_mpf_gdma", ZXDH_MPF_GDMA_IRQ_MIN,
...
s32 dh_mpf_irq_table_create(struct dh_core_dev *dev)
{
	...
	if (total_vec < 0) {
		DH_LOG_ERR(MODULE_MPF, "pci_alloc_irq_vectors failed: %d\n", total_vec);
		return total_vec;
	}
...
struct dh_irq *dh_mpf_async_irq_request(struct dh_core_dev *dev)
{
	...
	if (IS_ERR_OR_NULL(irq))
		DH_LOG_ERR(MODULE_MPF, "irq=0x%llx\r\n", (unsigned long long)irq);
	DH_LOG_DEBUG(MODULE_MPF, "end\r\n");
	return irq;
}
