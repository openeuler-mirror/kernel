 ...
 * Description   : FW type definitions
 */
...
typedef struct {
	u32 invalid;                                   // Non-0 means state machine does not exist, 0 means exists
	u32 cfg_index;                                 // Configuration file index, 0~7
	u8 image_state[UP_MAX_FW_TYPE_CNT];            // Master/backup state, 0 means master, 1 means backup
