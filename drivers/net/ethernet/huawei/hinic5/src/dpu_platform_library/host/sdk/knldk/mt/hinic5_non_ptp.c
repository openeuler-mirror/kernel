 ...
 * Description   : Non-PTP time synchronization implementation
 */
...
static int hinic5_non_ptp_mem_alloc(struct hinic5_hwdev *hwdev)
{
	...
	g_non_ptp_info = (struct hinic5_non_ptp_info *)(uintptr_t)__get_free_pages(GFP_KERNEL, NON_PTP_BUF_PAGE_ORDER);
	if (!g_non_ptp_info) {
...
static int hinic5_non_ptp_info_init(struct hinic5_hwdev *hwdev, u32 chip_id)
{
	...
	/* Initialize the non_ptp_info corresponding to the current chip */
...
static void hinic5_non_ptp_info_deinit(struct card_node *chip_node)
{
	...
	/* If ref_cnt is not 0 after decrement by 1, the interface returns false, return early */
	if (!atomic_sub_and_test(1, &chip_node->non_ptp_info->ref_cnt)) {
		return;
	}

	(void)memset(chip_node->non_ptp_info, 0,
...
static int hinic5_non_ptp_para_check(struct hinic5_hwdev *hwdev, u32 *chip_id)
{
	...
	/* Get the chip number, used to index non ptp information */
...
int hinic5_non_ptp_cdev_init(struct hinic5_hwdev *hwdev)
{
	...
	/* The following flow is only involved in the first initialization */
	atomic_inc(&g_non_ptp_cdev_ref_cnt);
...
/* Non ptp function destroy main function */
void hinic5_non_ptp_cdev_deinit(struct hinic5_hwdev *hwdev)
{
	...
	/* Destroy the non ptp info managed by the chip corresponding to the current function */
	hinic5_non_ptp_info_deinit(chip_node);
...
void hinic5_kernel_sync_time_work(struct work_struct *work)
{
	...
	struct hinic5_hwdev *hwdev = container_of(delay, struct hinic5_hwdev, sync_kernel_time_task);
	int err;
...
static inline bool support_non_ptp(struct hinic5_hwdev *hwdev)
{
	if ((hinic5_func_type(hwdev) != TYPE_PPF) || COMM_SUPPORT_NON_PTP_SYNC(hwdev) == 0) {
		return false;
	}
...
int hinic5_non_ptp_init(void *hwdev)
{
	...
	if (!support_non_ptp(dev)) {
		return 0;
	}

	err = hinic5_non_ptp_cdev_init(dev);
	if (err != 0) {
		sdk_err(dev->dev_hdl, "Failed to init non_ptp char dev\n");
		return err;
	}
	/* Register delayed work, and initialize as disable */
	INIT_DELAYED_WORK(&dev->sync_kernel_time_task, hinic5_kernel_sync_time_work);
...
void hinic5_non_ptp_uninit(void *hwdev)
{
	...
	cancel_delayed_work_sync(&dev->sync_kernel_time_task);
	hinic5_non_ptp_cdev_deinit(dev);
}
