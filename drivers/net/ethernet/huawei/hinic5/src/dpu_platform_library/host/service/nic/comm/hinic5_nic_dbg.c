 ...
 * Description   : hinic5 nic debug implementation
 */
			   ...
			   u32 msg_size)
{
	...
	sq_info->q_id = q_id;
	sq_info->pi = hinic5_get_sq_local_pi(sq);
	sq_info->ci = hinic5_get_sq_local_ci(sq);
	sq_info->fi = hinic5_get_sq_hw_ci(sq);
	sq_info->q_depth = sq->wq.q_depth;
	sq_info->wqebb_size = sq->wq.wqebb_size;
	sq_info->ci_wqe_page_addr = hinic5_wq_get_first_wqe_page_addr(&sq->wq);
...
{
	struct hinic5_nic_io *nic_io = NULL;
	struct hinic5_io_queue *rq = NULL;
	...
	rq_info->q_id = q_id;
	rq_info->hw_ci = hinic5_get_rq_hw_ci(rq);
	...
	rq_info->ci_wqe_page_addr = hinic5_wq_get_first_wqe_page_addr(&rq->wq);
