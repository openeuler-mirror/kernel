 ...
 * File Name     : cqm_object_intern.c
 * Version       : Initial Draft
 * Created       : 2026/5/20
 * Last Modified : 2026/09/16
 * Description   : CQM object internal implementation
 */
			...
			struct tag_cqm_queue *common)
{
	...
	if (srqtail_container) {
		...
		srqtail_container = (u8 *)(uintptr_t)CQM_ADDR_COMBINE(srq_link_wqe->fixed_next_buffer_addr_h,
	...
	do {
		...
		srqhead_container = (u8 *)(uintptr_t)CQM_ADDR_COMBINE(srq_link_wqe->fixed_next_buffer_addr_h,
							    srq_link_wqe->fixed_next_buffer_addr_l);
...
static void cqm_update_srq_link_wqe(u32 link_wqe_offset, u8 *new_container, dma_addr_t new_container_pa)
{
	struct tag_cqm_srq_linkwqe *srq_link_wqe = (struct tag_cqm_srq_linkwqe *)((uintptr_t)new_container + link_wqe_offset);
	...
	srq_link_wqe->current_buffer_addr_h = CQM_ADDR_HI((uintptr_t)new_container);
	srq_link_wqe->current_buffer_addr_l = CQM_ADDR_LW((uintptr_t)new_container);
...
s32 cqm_container_create(struct tag_cqm_object *object, u8 **container_addr, bool link)
{
	...
	struct tag_cqm_nonrdma_qinfo *qinfo = container_of(common, struct tag_cqm_nonrdma_qinfo, common);
	u32 link_wqe_offset = qinfo->wqe_per_buf * qinfo->wqe_size;
	struct hinic5_hwdev *handle = cqm_handle->ex_handle;
	struct tag_cqm_srq_linkwqe *srq_link_wqe = NULL;
	...
	new_container_pa = dma_map_single(cqm_handle->dev, new_container, qinfo->container_size, DMA_BIDIRECTIONAL);
	...
	if (link) {
		if (common->tail_container) {
			srq_link_wqe = (struct tag_cqm_srq_linkwqe *)(common->tail_container + link_wqe_offset);
			link_wqe = &srq_link_wqe->linkwqe;
			link_wqe->next_page_gpa_h = __swab32((u32)CQM_ADDR_HI(new_container_pa));
			link_wqe->next_page_gpa_l = __swab32((u32)CQM_ADDR_LW(new_container_pa));
			link_wqe->next_buffer_addr_h = __swab32((u32)CQM_ADDR_HI((uintptr_t)new_container));
			link_wqe->next_buffer_addr_l = __swab32((u32)CQM_ADDR_LW((uintptr_t)new_container));
			 ...
			 */
			wmb();
			srq_link_wqe->fixed_next_buffer_addr_h = (u32)CQM_ADDR_HI((uintptr_t)new_container);
			srq_link_wqe->fixed_next_buffer_addr_l = (u32)CQM_ADDR_LW((uintptr_t)new_container);
		}
	}
	...
	cqm_update_srq_link_wqe(link_wqe_offset, new_container, new_container_pa);
	if (link)
...
void cqm_share_recv_queue_delete(struct tag_cqm_object *object)
{
	...
	if (unlikely(common->q_ctx_vaddr == NULL)) {
		CQM_PTR_CHECK_ERR("Srq del: srqc kfree, context virtual addr is null\n");
		return;
	}
				    ...
				    u32 xid, u32 count)
{
	...
	if (CQM_DYNAMIC_XID_ALLOC_MODE(xid)) {
		if (CQM_DYNAMIC_XID_LB_MODE(xid) != CQM_XID_LOW_BIT_NONE ||
			...
			if (count > 1) {
				cqm_warn(handle->dev_hdl, "Not support alloc multiple bits.\n");
				return CQM_INDEX_INVALID;
			}
			index = cqm_bitmap_alloc_lowbits_align(bitmap, bp_range, cqm_handle,
		...
		if (index >= bitmap->max_num - bitmap->reserved_back) {
			cqm_warn(handle->dev_hdl, "cqm bitmap alloc, index is err. index:0x%x, max_num:0x%x, reserved_back:0x%x\n",
		...
		if (index != xid) {
			cqm_warn(handle->dev_hdl, "cqm bitmap alloc, index(0x%x) != xid(0x%x).\n", index, xid);
			return CQM_INDEX_INVALID;
		}
	}
				    ...
				    struct tag_cqm_bitmap_range *bp_range)
{
	...
	qpc_mpt_info->common.xid = cqm_general_bitmap_alloc(object, cla_table, bp_range,
...
{
	struct tag_cqm_qpc_mpt *common = container_of(object, struct tag_cqm_qpc_mpt, object);
	struct tag_cqm_qpc_mpt_info *qpc_mpt_info = container_of(common, struct tag_cqm_qpc_mpt_info, common);
	struct tag_cqm_handle *cqm_handle = (struct tag_cqm_handle *)object->cqm_handle;
	struct hinic5_hwdev *handle = cqm_handle->ex_handle;
	...
	if (object->object_type == CQM_OBJECT_SERVICE_CTX) {
		cla_table = cqm_cla_table_get(&cqm_handle->bat_table, CQM_BAT_ENTRY_T_QPC);
	} else if (object->object_type == CQM_OBJECT_MPT) {
		cla_table = cqm_cla_table_get(&cqm_handle->bat_table, CQM_BAT_ENTRY_T_MPT);
	} else {
	...
	if (cqm_qpc_mpt_bitmap_alloc(object, cla_table, bp_range) == CQM_FAIL) {
		cqm_warn(handle->dev_hdl, CQM_FUNCTION_FAIL(cqm_qpc_mpt_bitmap_alloc));
		return CQM_FAIL;
	}
	...
	if (!CQM_IS_FAKE_CHILD(cqm_handle)) {
		...
		qpc_mpt_info->common.vaddr = cqm_cla_get(cqm_handle, cla_table,
	...
	if (cqm_object_table_insert(cqm_handle, object_table, index, object, is_lock_bh) != CQM_SUCCESS) {
		cqm_err(handle->dev_hdl, CQM_FUNCTION_FAIL(cqm_object_table_insert));
		goto err2;
	}
...
void cqm_qpc_mpt_delete(struct tag_cqm_object *object)
{
	 ...
	 */
	if (!cla_table->alloc_static || object->service_type == CQM_SERVICE_T_ROCE ||
...
static int cqm_nonrdma_queue_ctx_create_scq(struct tag_cqm_object *object)
{
	...
	common->q_ctx_vaddr = cqm_cla_get(cqm_handle, cla_table, qinfo->common.index,
...
static s32 cqm_nonrdma_queue_ctx_create(struct tag_cqm_object *object)
{
	...
	if (object->object_type == CQM_OBJECT_NONRDMA_SRQ) {
		...
		common->q_ctx_paddr = dma_map_single(cqm_handle->dev, common->q_ctx_vaddr,
						     qinfo->q_ctx_size, DMA_BIDIRECTIONAL);
		if (dma_mapping_error(cqm_handle->dev, common->q_ctx_paddr) != 0) {
...
static void cqm_free_queue_header(struct tag_cqm_queue *common, struct tag_cqm_handle *cqm_handle,
	struct hinic5_hwdev *handle)
{
	dma_unmap_single(cqm_handle->dev, common->q_header_paddr, sizeof(struct tag_cqm_queue_header),
		DMA_BIDIRECTIONAL);
...
static s32 cqm_alloc_queue_header(struct tag_cqm_queue *common, struct tag_cqm_handle *cqm_handle,
	struct hinic5_hwdev *handle)
{
	common->q_header_vaddr = cqm_kmalloc_align(sizeof(struct tag_cqm_queue_header),
	...
	common->q_header_paddr = dma_map_single(cqm_handle->dev, common->q_header_vaddr,
...
s32 cqm_nonrdma_queue_create(struct tag_cqm_object *object)
{
	 ...
	 */
	if (object->object_type == CQM_OBJECT_NONRDMA_EMBEDDED_CQ || object->object_type == CQM_OBJECT_NONRDMA_SCQ) {
		/* depth: 2^n-aligned; depth range: 256-32 K */
		if (wqe_number < CQM_CQ_DEPTH_MIN || wqe_number > CQM_CQ_DEPTH_MAX) {
	 ...
	 * link wqe
	 */
	cqm_linkwqe_fill(q_room_buf, qinfo->wqe_per_buf, wqe_size, wqe_number - buf_number, tail, common->queue_link_mode);
	...
	if (cqm_nonrdma_queue_ctx_create(object) == CQM_FAIL) {
		cqm_err(handle->dev_hdl, CQM_FUNCTION_FAIL(cqm_nonrdma_queue_ctx_create));
		goto err2;
	}
...
void cqm_nonrdma_queue_delete(struct tag_cqm_object *object)
{
	...
	if (object->object_type == CQM_OBJECT_NONRDMA_SCQ) {
		cla_table = cqm_cla_table_get(bat_table, CQM_BAT_ENTRY_T_SCQC);
		if (unlikely(cla_table == NULL)) {
			CQM_PTR_CHECK_ERR(CQM_FUNCTION_FAIL(cqm_cla_table_get_queue));
			return;
		}

		/* disassociate index and object */
		object_table = &cla_table->obj_table;
		cqm_object_table_remove(cqm_handle, object_table, index, object, object->service_type != CQM_SERVICE_T_FC);
	}
	...
	if (object->object_type == CQM_OBJECT_NONRDMA_SRQ) {
		 ...
		 */
		if (common->q_ctx_vaddr) {
			dma_unmap_single(cqm_handle->dev, common->q_ctx_paddr, qinfo->q_ctx_size, DMA_BIDIRECTIONAL);
...
static s32 cqm_rdma_queue_ctx_create(struct tag_cqm_object *object, struct tag_cqm_bitmap_range *bp_range)
{
	struct tag_cqm_queue *common = container_of(object, struct tag_cqm_queue, object);
	struct tag_cqm_rdma_qinfo *qinfo = container_of(common, struct tag_cqm_rdma_qinfo, common);
	struct tag_cqm_handle *cqm_handle = (struct tag_cqm_handle *)object->cqm_handle;
	struct tag_cqm_bat_table *bat_table = &cqm_handle->bat_table;
	struct hinic5_hwdev *handle = cqm_handle->ex_handle;
	struct tag_cqm_object_table *object_table = NULL;
	...
	if (object->object_type == CQM_OBJECT_RDMA_SRQ || object->object_type == CQM_OBJECT_RDMA_SCQ) {
		...
		if (!cla_table) {
			cqm_err(handle->dev_hdl, CQM_FUNCTION_FAIL(rdma_cqm_cla_table_get));
			return CQM_FAIL;
		}

		qinfo->index_count = (ALIGN(qinfo->q_ctx_size, cla_table->obj_size)) / cla_table->obj_size;
		qinfo->common.index = cqm_general_bitmap_alloc(object, cla_table, bp_range, qinfo->common.index,
			qinfo->index_count);
		if (qinfo->common.index == CQM_INDEX_INVALID) {
			cqm_err(handle->dev_hdl, CQM_FUNCTION_FAIL(cqm_general_bitmap_alloc));
			return CQM_FAIL;
		}
		...
		object_table = &cla_table->obj_table;
		if (cqm_object_table_insert(cqm_handle, object_table, qinfo->common.index, object, true) != CQM_SUCCESS) {
			cqm_err(handle->dev_hdl, CQM_FUNCTION_FAIL(rdma_cqm_object_table_insert));
			cqm_cla_put(cqm_handle, cla_table, qinfo->common.index, qinfo->index_count);
			cqm_bitmap_free(bitmap, qinfo->common.index, qinfo->index_count);
			return CQM_FAIL;
		}
...
static s32 cqm_qinfo_judgment(struct tag_cqm_rdma_qinfo *qinfo, struct tag_cqm_buf *q_room_buf,
...
s32 cqm_rdma_queue_create(struct tag_cqm_object *object, struct tag_cqm_bitmap_range *bp_range)
{
	...
	if (qinfo->room_header_alloc) {
		...
		if (cqm_qinfo_judgment(qinfo, q_room_buf, cqm_handle, handle) == CQM_FAIL) {
			return CQM_FAIL;
		}
	}
...
void cqm_rdma_queue_delete(struct tag_cqm_object *object)
{
	...
	if (object->object_type == CQM_OBJECT_RDMA_SCQ || object->object_type == CQM_OBJECT_RDMA_SRQ) {
...
s32 cqm_rdma_table_create(struct tag_cqm_object *object)
{
	...
	if (object->object_size <= PAGE_SIZE ||
		...
		buf->direct.va = dma_alloc_coherent(cqm_handle->dev, buf->buf_size,
