 ...
 * File Name     : cqm_bat_cla.c
 * Version       : Initial Draft
 * Created       : 2026/5/20
 * Last Modified : 2026/09/16
 * Description   : CQM BAT and CLA table management implementation
 */
...
static bool cqm_cla_hugepage_hint;
module_param(cqm_cla_hugepage_hint, bool, 0444);
MODULE_PARM_DESC(cqm_cla_hugepage_hint,
					...
					struct tag_cqm_bat_entry_standerd *bat_entry_standerd)
{
	...
	cqm_info(cqm_handle->dev, "Bat fill: cla_type %u, pa 0x%llx, gpa 0x%x-0x%x, level %u\n",
...
static void cqm_bat_fill_cla(struct tag_cqm_handle *cqm_handle)
{
	...
	for (i = 0; i < CQM_BAT_ENTRY_MAX; i++) {
		...
		if (entry_type == CQM_BAT_ENTRY_T_CFG) {
			...
			if (CQM_IS_PPF(cqm_handle) && CQM_IS_LB_MODE_1_OR_2(cqm_handle)) {
			...
			if (COMM_SUPPORT_VIRTIO_FC_CACHE(hwdev) && CQM_IS_LB_MODE_1_OR_2(cqm_handle)) {
				  ...
				  struct tag_cqm_bat_update_param *param)
{
	...
	illegal_args = (param->bat_offset % CQM_BAT_ENTRY_SIZE != 0) ||
	...
	if (ret != CQM_SUCCESS) {
		cqm_err(handle->dev_hdl, CQM_FUNCTION_FAIL(prepare_cmd_buf_bat_update));
		return CQM_FAIL;
	}

	cqm_dbg_byte_print(handle->dev_hdl, (u32 *)bat_table->bat, sizeof(bat_table->bat));

	ret = cqm5_send_cmd_box((void *)(handle), CQM_MOD_CQM,
			      ...
			      u32 smf_id, u32 func_id)
{
	...
	if (bat_table->bat_size > CQM_BAT_MAX_SIZE) {
		cqm_err(handle->dev_hdl, "bat_size = %u, which is more than %d.",
	...
	if (unlikely(ret != CQM_SUCCESS)) {
		cqm_err(handle->dev_hdl, CQM_FUNCTION_FAIL(hinic5_bat_get_l3i_entry_config));
		return ret;
	}
	...
	if (l3i_config.mapping &&
		...
		ret = cqm_bat_update_smf_cmd(cqm_handle, buf_in, &param);
		if (ret != CQM_SUCCESS)
			goto cmd_err;
...
static s32 cqm_bat_update(struct tag_cqm_handle *cqm_handle)
{
	...
	if (CQM_IS_FAKE_CHILD(cqm_handle)) {
		cqm_err(hwdev->dev_hdl, "Bat update: unsupported for fake child\n");
		return CQM_FAIL;
	}
...
s32 cqm_bat_init(struct tag_cqm_handle *cqm_handle)
{
	...
	(void)memset(bat_table, 0, sizeof(struct tag_cqm_bat_table));
...
STATIC s32 cqm_cla_reset(struct tag_cqm_handle *cqm_handle)
{
	...
	cqm_swab32((u8 *)cmd_data, sizeof(struct tag_cqm_cla_reset_cmd) >> CQM_DW_SHIFT);
...
static u64 cqm_cla_chip_gpa_flags(struct tag_cqm_handle *cqm_handle, u8 gpa_check_enable)
{
	struct hinic5_func_attr *func_attr = NULL;
...
#define CQM_TIMER_FUNC_BUF_NUM 64
		cqm_dbg_on(i % CQM_TIMER_FUNC_BUF_NUM == 0, handle->dev_hdl,
		...
		*base = (dma_addr_t)(((u64)(cla_sub_buf->buf_list[index_sub_offset].pa) & CQM_CHIP_GPA_MASK)
			| gpa_flags);
...
static void cqm_apply_new_buf(struct tag_cqm_buf *cla_buf, u32 buf_size, u32 buf_num, u32 buf_order)
{
...
static s32 cqm_cla_xyz_lvl0(struct tag_cqm_handle *cqm_handle, struct tag_cqm_cla_table *cla_table, u32 trunk_size)
{
	...
	if (secure_mem_en && CQM_IS_VF(cqm_handle) && CQM_CLA_IS_SECURE_MEM(cla_table->type))
...
static s32 cqm_cla_xyz_lvl1(struct tag_cqm_handle *cqm_handle, struct tag_cqm_cla_table *cla_table, u32 trunk_size)
{
	struct hinic5_hwdev *handle = cqm_handle->ex_handle;
	...
	cqm_apply_new_buf(cla_z_buf, trunk_size, z_buf_num, cla_table->trunk_order);
	/* All buffer space must be statically allocated. */
	if (cla_table->alloc_static) {
		ret = cqm_cla_fill_buf(cqm_handle, cla_y_buf, cla_z_buf, gpa_check_enable);
		if (unlikely(ret != CQM_SUCCESS)) {
			cqm_warn(handle->dev_hdl, CQM_FUNCTION_FAIL(cqm_cla_fill_buf));
				      ...
				      struct hinic5_hwdev *handle)
{
	...
	ret = snprintf(cla_x_buf->buf_info.buf_vram_name, VRAM_NAME_MAX_LEN,
		       "%s%s", cla_table->name, VRAM_CQM_CLA_COORD_X);
	if (ret < 0) {
		cqm_err(handle->dev_hdl, "cqm cla x vram name snprintf failed, cla_table->name:%s", cla_table->name);
		return CQM_FAIL;
	}

	cla_y_buf->buf_info.use_vram = use_vram;
	ret = snprintf(cla_y_buf->buf_info.buf_vram_name, VRAM_NAME_MAX_LEN,
		       "%s%s", cla_table->name, VRAM_CQM_CLA_COORD_Y);
	if (ret < 0) {
		cqm_err(handle->dev_hdl, "cqm cla y vram name snprintf failed");
		return CQM_FAIL;
	}

	cla_z_buf->buf_info.use_vram = use_vram;
	ret = snprintf(cla_z_buf->buf_info.buf_vram_name, VRAM_NAME_MAX_LEN,
		       "%s%s", cla_table->name, VRAM_CQM_CLA_COORD_Z);
	if (ret < 0) {
		cqm_err(handle->dev_hdl, "cqm cla z vram name snprintf failed");
		return CQM_FAIL;
	}
					    ...
					    u32 trunk_size)
{
	...
	if (unlikely(cqm_buf_alloc(cqm_handle, cla_y_buf, false) != CQM_SUCCESS)) {
		cqm_err(handle->dev_hdl, CQM_ALLOC_FAIL(lvl_2_y_buf));
		cqm_buf_free(cla_x_buf, cqm_handle->dev);
		return CQM_FAIL;
	}
	...
	if (unlikely(cqm_buf_alloc(cqm_handle, cla_z_buf, false) != CQM_SUCCESS)) {
		cqm_err(handle->dev_hdl, CQM_ALLOC_FAIL(lvl_2_z_buf));
		cqm_buf_free(cla_y_buf, cqm_handle->dev);
		cqm_buf_free(cla_x_buf, cqm_handle->dev);
		return CQM_FAIL;
				  ...
				  u32 trunk_size)
{
	...
	if (cqm_cla_xyz_lvl2_timer_xyz_apply(cqm_handle, cla_table, trunk_size) != CQM_SUCCESS)
		return CQM_FAIL;

	func_timer_size = CQM_TIMER_ALIGN_SCALE_NUM * func_cap->timer_basic_size;
			     ...
			     u32 order)
{
	...
	if (cla_lvl == CQM_CLA_LVL_0) {
		...
		if (unlikely(ret != CQM_SUCCESS))
			cqm_warn(cqm_handle->dev, CQM_FUNCTION_FAIL(cqm_cla_xyz_lvl0));
		return ret;
	}
	...
	if (cla_lvl == CQM_CLA_LVL_1) {
		...
		if (unlikely(ret != CQM_SUCCESS))
			cqm_warn(cqm_handle->dev, CQM_FUNCTION_FAIL(cqm_cla_xyz_lvl1));
		return ret;
	}
	...
	if (cla_lvl == CQM_CLA_LVL_2) {
		if (cla_table->type == CQM_BAT_ENTRY_T_TIMER) {
			ret = cqm_cla_xyz_lvl2_timer(cqm_handle, cla_table, buf_size);
			if (unlikely(ret != CQM_SUCCESS))
				cqm_warn(cqm_handle->dev, CQM_FUNCTION_FAIL(cqm_cla_xyz_lvl2_timer));
			return ret;
		} else {
			ret = cqm_cla_xyz_lvl2(cqm_handle, cla_table, buf_size);
			if (unlikely(ret != CQM_SUCCESS))
				cqm_warn(cqm_handle->dev, CQM_FUNCTION_FAIL(cqm_cla_xyz_lvl2));
			return ret;
		}
	}
				...
				struct tag_cqm_cla_table *cla_table)
{
	...
	for (order = max_order; order >= min_order; order--) {
		...
		if (unlikely(ret != CQM_SUCCESS))
			cqm_err(cqm_handle->dev, CQM_FUNCTION_FAIL(cqm_cla_xyz_alloc));
		return ret;
	}
					    ...
					    struct tag_cqm_func_capability *capability)
{
	...
	if (!secure_mem_en || !CQM_IS_VF(cqm_handle))
		return;
	...
	cqm_info(handle->dev_hdl, "Secure mem: cla_type=%u, max_buffer_size=0x%x, order=%u\n",
			       ...
			       struct tag_cqm_func_capability *capability)
{
	...
	update_entry_cap_for_secure_mem(cqm_handle, cla_table, capability);
}
			       ...
			       struct tag_cqm_func_capability *capability)
{
	...
	cla_table->max_buffer_size = cla_table->obj_size * cla_table->obj_num;
	cla_table->trunk_order = cqm_shift(ALIGN(cla_table->max_buffer_size, PAGE_SIZE) / PAGE_SIZE);
	cla_table->alloc_static = true;
}
...
static void cqm_cla_init_entry_capability(struct tag_cqm_handle *cqm_handle,
					  struct tag_cqm_cla_table *cla_table,
					  struct tag_cqm_func_capability *capability)
{
	...
	if (cla_table->type < ARRAY_SIZE(init_entry_cap_funcs) &&
		...
		init_entry_cap_funcs[cla_table->type](cqm_handle, cla_table, capability);
}

static s32 cqm_cla_init_entry_memory(struct tag_cqm_handle *cqm_handle, u32 entry_idx)
{
	struct hinic5_hwdev *hwdev = cqm_handle->ex_handle;
	struct tag_cqm_bat_table *bat_table = &cqm_handle->bat_table;
	 ...
	 */
	if (CQM_IS_LB_MODE_1_OR_2(cqm_handle) &&
	    (entry_type == CQM_BAT_ENTRY_T_TIMER || entry_type == CQM_BAT_ENTRY_T_HASH ||
	     (entry_type == CQM_BAT_ENTRY_T_XID2CID && COMM_SUPPORT_VIRTIO_FC_CACHE(hwdev)))) {
		for (i = 0; i < cqm_handle->func_capability.smf_max_num; i++) {
			...
			(void)memcpy(cla_table_tmp, cla_table,
				     sizeof(struct tag_cqm_cla_table));

			ret = snprintf(cla_table_tmp->name, VRAM_NAME_MAX_LEN,
				       "%s%s%01u", cla_table->name,
				       VRAM_CQM_CLA_SMF_BASE, i);
			if (ret < 0) {
				cqm_err(cqm_handle->ex_handle->dev_hdl,
					"cqm cla timer vram name snprintf failed");
			      ...
			      struct tag_cqm_func_capability *capability)
{
	...
	for (i = 0; i < CQM_BAT_ENTRY_MAX; i++) {
		...
		err = snprintf(cla_table->name, VRAM_NAME_MAX_LEN,
			"%s%s%s%02u", cqm_handle->name, VRAM_CQM_CLA_BASE,
			VRAM_CQM_CLA_TYPE_BASE, cla_table->type);
		if (err < 0) {
			cqm_err(cqm_handle->ex_handle->dev_hdl,
				"cqm cla table vram name snprintf failed");
			return CQM_FAIL;
		}

		mutex_init(&cla_table->lock);

		cqm_cla_init_entry_capability(cqm_handle, cla_table, capability);
...
static u32 cqm_cla_get_ctx_mem_size(struct tag_cqm_func_capability *capability, u32 cla_type)
{
	u32 basic_size;
	u32 num;
...
static s32 cqm_stub_get_func_secure_mem_size(struct tag_cqm_handle *cqm_handle,
					     struct tag_cqm_func_capability *capability,
					     u32 *total_size)
{
	struct hinic5_hwdev *handle = cqm_handle->ex_handle;
	u32 cla_type[] = { CQM_BAT_ENTRY_T_SCQC,
			   CQM_BAT_ENTRY_T_QPC,
			   CQM_BAT_ENTRY_T_MPT,
	...
	/* mem size check */
	for (i = 0; i < cla_num; i++) {
		mem_size = cqm_cla_get_ctx_mem_size(capability, cla_type[i]);
		if (!CQM_IS_SECURE_MEMSIZE_VALID(mem_size)) {
			cqm_err(handle->dev_hdl,
...
/* secure mem is currently stub-allocated by sdk; in production, it is allocated by QEMU and obtained by reading the SML table */
static s32 cqm_stub_alloc_secure_mem(struct tag_cqm_handle *cqm_handle,
				     struct tag_cqm_func_capability *capability)
{
	struct cqm_secure_mem_info *secure_mem_info = &cqm_handle->bat_table.func_secure_mem;
	struct hinic5_hwdev *handle = cqm_handle->ex_handle;
	struct device *dev = cqm_handle->dev;
	u32 secure_mem_size = 0;
	u32 ret, order, map_size;

	ret = cqm_stub_get_func_secure_mem_size(cqm_handle, capability, &secure_mem_size);
	if (ret != CQM_SUCCESS)
		return ret;
	...
	cqm_info(handle->dev_hdl, "total_mem_size=0x%x, map_size=0x%x, order=%d\n",
	...
	if (!secure_mem_info->addr.va) {
		cqm_err(handle->dev_hdl, CQM_ALLOC_FAIL(secure_mem));
		return CQM_FAIL;
	}
	...
	if (dma_mapping_error(dev, secure_mem_info->addr.pa)) {
		cqm_err(handle->dev_hdl, CQM_MAP_FAIL(secure_mem));
...
/* Obtain the VF secure memory address from the SML table and map it */
static s32 cqm_cla_get_secure_mem(struct tag_cqm_handle *cqm_handle)
{
	struct cqm_secure_mem_info *secure_mem_info = &cqm_handle->bat_table.func_secure_mem;
	struct hinic5_hwdev *handle = cqm_handle->ex_handle;
	s32 ret;
	...
	if (ret == CQM_CONTINUE) {
		secure_mem_en = false;
		return CQM_SUCCESS;
	} else if (ret != CQM_SUCCESS) {
		cqm_err(handle->dev_hdl, "failed to get secure mem, ret: %d, func_id %hu\n",
...
#if defined(__UEFI__) || defined(SECURE_MEM_STUB)
/* secure mem is currently stub-allocated by sdk; free memory if ctx size is invalid; in production, freed by QEMU */
static void cqm_free_secure_mem(struct tag_cqm_handle *cqm_handle)
{
	struct cqm_secure_mem_info *secure_mem_info = &cqm_handle->bat_table.func_secure_mem;
	struct device *dev = cqm_handle->dev;
	u32 order;
...
static struct tag_cqm_cla_table *cqm_cla_get_entry(struct tag_cqm_handle *cqm_handle, u32 cla_type)
{
	u32 *bat_entry_type = cqm_handle->bat_table.bat_entry_type;
	struct tag_cqm_cla_table *cla_entry = cqm_handle->bat_table.entry;
	u32 i;
...
static s32 cqm_cla_secure_mem_assign(struct tag_cqm_handle *cqm_handle, struct tag_cqm_func_capability *capability,
				     u32 cla_type, u32 *mem_offset)
{
	struct cqm_secure_mem_info *secure_mem_info = &cqm_handle->bat_table.func_secure_mem;
	struct hinic5_hwdev *handle = cqm_handle->ex_handle;
	struct tag_cqm_cla_table *cla_entry = NULL;
	u32 cla_mem_size;

	cla_entry = cqm_cla_get_entry(cqm_handle, cla_type);
	if (!cla_entry) {
		cqm_err(handle->dev_hdl, "Get cla entry failed: cla_type=%u\n", cla_type);
		return CQM_FAIL;
	}
	cla_entry->secure_mem.va = secure_mem_info->addr.va + *mem_offset;
	cla_entry->secure_mem.pa = secure_mem_info->addr.pa + *mem_offset;

	cla_mem_size = cqm_cla_get_ctx_mem_size(capability, cla_type);

	*mem_offset += cla_mem_size;
	if (*mem_offset >= secure_mem_info->len) {
		cqm_err(handle->dev_hdl, "Mem size exceeds: mem_offset=0x%x, max_size=0x%x\n",
...
static s32 cqm_cla_secure_mem_init(struct tag_cqm_handle *cqm_handle)
{
	struct tag_cqm_func_capability *capability = &cqm_handle->func_capability;
	struct hinic5_hwdev *handle = cqm_handle->ex_handle;
	...
	/* secure mem is currently stub-allocated by sdk; in production, it is allocated by QEMU and obtained by reading the VF BAR space */
#if defined(__UEFI__) || defined(SECURE_MEM_STUB)
	if (cqm_stub_alloc_secure_mem(cqm_handle, capability) != CQM_SUCCESS)
#else
	if ((cqm_cla_get_secure_mem(cqm_handle)) != CQM_SUCCESS)
#endif
		return CQM_FAIL;
	...
	for (i = 0; i < cla_num; i++) {
		ret = cqm_cla_secure_mem_assign(cqm_handle, capability, cla_type[i], &mem_offset);
		if (ret != CQM_SUCCESS) {
			cqm_err(handle->dev_hdl, "Cla secure mem assign failed: cla_type=%u\n", cla_type[i]);
...
s32 cqm_cla_init(struct tag_cqm_handle *cqm_handle)
{
	...
	if (secure_mem_en && CQM_IS_VF(cqm_handle)) {
		if (unlikely(cqm_cla_secure_mem_init(cqm_handle) != CQM_SUCCESS)) {
	...
	if (CQM_IS_PPF(cqm_handle) &&
		...
		if (ret != CQM_SUCCESS) {
			cqm_err(handle->dev_hdl, "PPF timer start failed, err %d\n", ret);
			goto err;
		}
	}
...
void cqm_cla_uninit(struct tag_cqm_handle *cqm_handle, u32 entry_numb)
{
	...
	if (CQM_IS_LB_MODE_1_OR_2(cqm_handle) && COMM_SUPPORT_VIRTIO_FC_CACHE(hwdev)) {
		for (i = 0; i < cqm_handle->func_capability.smf_max_num; i++) {
	...
	/* Free secure memory. In production, freed by QEMU */
	cqm_free_secure_mem(cqm_handle);
}
			      ...
			      cqm_cla_update_cmd_s *cmd_info)
{
	...
	cqm_handle->cmdq_ops->prepare_cmd_buf_cla_update(cmd_info, buf_in, &cmd);
	ret = cqm5_send_cmd_box((void *)(cqm_handle->ex_handle), CQM_MOD_CQM,
			       cmd, buf_in, NULL, NULL,
			       CQM_CMD_TIMEOUT, HINIC5_CHANNEL_DEFAULT);
	if (ret != CQM_SUCCESS) {
		cqm_err(handle->dev_hdl, CQM_FUNCTION_FAIL(cqm5_send_cmd_box));
		cqm_err(handle->dev_hdl, "Cla alloc: cqm_cla_update, cqm5_send_cmd_box_ret=%d\n",
			ret);
...
static void cqm_cla_cmd_init(cqm_cla_update_cmd_s *cmd, struct tag_cqm_handle *cqm_handle, dma_addr_t parant_pa,
	dma_addr_t child_pa, u8 cla_update_mode)
{
	...
	if (gpa_check_enable != 0) {
		...
		case CQM_CLA_DEL_GPA_WITH_CACHE_INVALID:
			...
			break;
		default:
			cqm_err(handle->dev_hdl, "Cla alloc: %s, wrong cla_update_mode=%u\n", __func__, cla_update_mode);
			break;
		}
	}
}
 ...
 *   Modification : Created function
 */
STATIC s32 cqm_cla_update(struct tag_cqm_handle *cqm_handle, const struct tag_cqm_buf_list *buf_node_parent,
	const struct tag_cqm_buf_list *buf_node_child, u32 child_index, u8 cla_update_mode)
{
	struct hinic5_hwdev *handle = cqm_handle->ex_handle;
	struct tag_cqm_cmd_buf *buf_in = NULL;
	cqm_cla_update_cmd_s cmd;
	...
	cqm_cla_cmd_init(&cmd, cqm_handle, (buf_node_parent->pa + (child_index * sizeof(dma_addr_t))), buf_node_child->pa,
		cla_update_mode);
...
static void cqm_unmap_and_free_pages(struct tag_cqm_handle *cqm_handle, struct tag_cqm_buf_list *buf_node, u32 order)
{
	/* Remove PCI mapping from the trunk page */
	dma_unmap_single(cqm_handle->dev, buf_node->pa, PAGE_SIZE << order, DMA_BIDIRECTIONAL);
 ...
 *   Modification : Created function
 */
static void cqm_cla_free_without_cache_invalid(struct tag_cqm_handle *cqm_handle,
			 struct tag_cqm_cla_table *cla_table,
			 struct tag_cqm_buf_list *buf_node_parent,
			 ...
			 u32 child_index)
{
	...
	if (cqm_cla_update(cqm_handle, buf_node_parent, buf_node_child,
		child_index, CQM_CLA_DEL_GPA_WITHOUT_CACHE_INVALID) != CQM_SUCCESS) {
		cqm_err(handle->dev_hdl, CQM_FUNCTION_FAIL(cqm_cla_update));
		return;
	}
	/* Remove PCI mapping from the trunk page and Release trunk page */
	cqm_unmap_and_free_pages(cqm_handle, buf_node_child, cla_table->trunk_order);
}

STATIC void cqm_cla_free_with_cache_invalid(struct tag_cqm_handle *cqm_handle,
			 struct tag_cqm_cla_table *cla_table,
			 struct tag_cqm_buf_list *buf_node_parent,
			 struct tag_cqm_buf_list *buf_node_child,
			 u32 child_index)
{
	...
	if (cqm_cla_update(cqm_handle, buf_node_parent, buf_node_child,
		child_index, CQM_CLA_DEL_GPA_WITH_CACHE_INVALID) != CQM_SUCCESS) {
		cqm_err(handle->dev_hdl, CQM_FUNCTION_FAIL(cqm_cla_update));
		return;
	}
	...
	if (cqm_cla_cache_invalid(cqm_handle, buf_node_child->pa, trunk_size) != CQM_SUCCESS) {
		cqm_err(handle->dev_hdl, CQM_FUNCTION_FAIL(cqm_cla_cache_invalid));
		return;
	}
				      ...
				      u32 index, u32 count, dma_addr_t *pa)
{
	...
	if (!buf_node_y->va) {
		if (cqm_cla_alloc(cqm_handle, cla_table, buf_node_x, buf_node_y, x_index) == CQM_FAIL) {
			cqm_err(handle->dev_hdl, CQM_FUNCTION_FAIL(cqm_cla_alloc));
			return NULL;
		}
	}
	...
	if (!buf_node_z->va) {
		if (cqm_cla_alloc(cqm_handle, cla_table, buf_node_y, buf_node_z, y_index) == CQM_FAIL) {
			cqm_err(handle->dev_hdl, CQM_FUNCTION_FAIL(cqm_cla_alloc));
			if (buf_node_y->refcount == 0)
				/* To release node Y, cache_invalid is
				 * required.
				 */
				cqm_cla_free_with_cache_invalid(cqm_handle, cla_table, buf_node_x, buf_node_y, x_index);
			return NULL;
		}
...
{
	if (cla_table->cla_lvl == CQM_CLA_LVL_0)
		return cqm_cla_do_get_lvl0(cqm_handle, cla_table, index, count, pa);
	if (cla_table->cla_lvl == CQM_CLA_LVL_1)
		return cqm_cla_do_get_lvl1(cqm_handle, cla_table, index, count, pa);
	if (cla_table->cla_lvl == CQM_CLA_LVL_2)
		return cqm_cla_do_get_lvl2(cqm_handle, cla_table, index, count, pa);
	WARN_ON(true);
	return NULL;
}
 ...
 *   Modification : Created function
 */
void cqm_cla_put(struct tag_cqm_handle *cqm_handle, struct tag_cqm_cla_table *cla_table, u32 index, u32 count)
{
	struct tag_cqm_buf *cla_z_buf = &cla_table->cla_z_buf;
	struct tag_cqm_buf *cla_y_buf = &cla_table->cla_y_buf;
	struct tag_cqm_buf *cla_x_buf = &cla_table->cla_x_buf;
	...
	if (cla_table->cla_lvl == CQM_CLA_LVL_1) {
		...
		if (y_index >= cla_z_buf->buf_number) {
			cqm_err(handle->dev_hdl, "Cla put: idx exceeds buf_number, y_idx %u, z_buf_num %u type %u\n",
		...
		if (buf_node_z->refcount == 0)
			...
			cqm_cla_free_without_cache_invalid(cqm_handle, cla_table, buf_node_y, buf_node_z, y_index);
