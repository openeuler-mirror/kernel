...
static u8 xor_chksum_set(void *data, u32 data_len)
{
	int idx;
	u8 checksum = 0;
	u8 *val = data;
	u8 max_idx = (u8)(data_len / sizeof(u8));

	for (idx = 0; idx < API_DESC_LEN && idx < max_idx; idx++)
		checksum ^= val[idx];

	return checksum;
}

static void set_prod_idx(struct hinic5_api_cmd_chain *chain)
{
...
static u32 get_hw_cons_idx(struct hinic5_api_cmd_chain *chain)
{
	u32 addr, val;
...
static void dump_api_chain_reg(struct hinic5_api_cmd_chain *chain)
{
 ...
 **/
static int chain_busy(struct hinic5_api_cmd_chain *chain)
{
	void *dev = chain->hwdev->dev_hdl;
	struct hinic5_api_cmd_cell_ctxt *ctxt = NULL;
	u64 resp_header;
 ...
 * @type: chain type
 **/
static u16 get_cell_data_size(enum hinic5_api_cmd_chain_type type)
{
...
static void prepare_cell_ctrl(u64 *cell_ctrl, u16 cell_len)
{
	...
	ctrl = HINIC5_API_CMD_CELL_CTRL_SET(SIZE_8BYTES(cell_len), CELL_LEN) |
	       HINIC5_API_CMD_CELL_CTRL_SET(0ULL, RD_DMA_ATTR_OFF) |
	       HINIC5_API_CMD_CELL_CTRL_SET(0ULL, WR_DMA_ATTR_OFF);

	chksum = xor_chksum_set(&ctrl, sizeof(ctrl));

	ctrl |= HINIC5_API_CMD_CELL_CTRL_SET(chksum, XOR_CHKSUM);
 ...
 **/
static void prepare_api_cmd(struct hinic5_api_cmd_chain *chain,
			    ...
			    const void *cmd, u16 cmd_size)
{
	...
	cell->desc |= HINIC5_API_CMD_DESC_SET(APICHN_0, APICHN_CODE) |
		      HINIC5_API_CMD_DESC_SET(APICHN_VALID, APICHN_RSVD);
 ...
 **/
static void prepare_cell(struct hinic5_api_cmd_chain *chain, u8 node_id,
			 const void *cmd, u16 cmd_size)
{
	struct hinic5_api_cmd_cell *curr_node = NULL;
	u16 cell_size;
...
static inline void cmd_chain_prod_idx_inc(struct hinic5_api_cmd_chain *chain)
{
	chain->prod_idx = MASKED_IDX(chain, chain->prod_idx + 1);
}

static void issue_api_cmd(struct hinic5_api_cmd_chain *chain)
{
	set_prod_idx(chain);
}
 ...
 **/
static void api_cmd_status_update(struct hinic5_api_cmd_chain *chain)
{
	...
	if (HINIC5_API_CMD_STATUS_GET(buf_desc, CHKSUM_ERR) != 0)
		return;

	status_header = be64_to_cpu(wb_status->header);
	chain_type = HINIC5_API_CMD_STATUS_HEADER_GET(status_header, CHAIN_ID);
	if (chain_type >= HINIC5_API_CMD_MAX)
		return;

	if (chain_type != chain->chain_type)
		return;

	chain->cons_idx = HINIC5_API_CMD_STATUS_GET(buf_desc, CONS_IDX);
}

static enum hinic5_wait_return wait_for_status_poll_handler(void *priv_data)
{
	struct hinic5_api_cmd_chain *chain = priv_data;

	if (!hinic5_is_chip_present(chain->hwdev))
...
static enum hinic5_wait_return check_cmd_resp_handler(void *priv_data)
{
	struct hinic5_api_cmd_cell_ctxt *ctxt = priv_data;
	u64 resp_header;
	u8 resp_status;

	if (!hinic5_is_chip_present(ctxt->hwdev))
		return WAIT_PROCESS_ERR;

	resp_header = be64_to_cpu(ctxt->resp->header);
	rmb(); /* read the latest header */

	if (HINIC5_API_CMD_RESP_HEADER_VALID(resp_header)) {
		resp_status = HINIC5_API_CMD_RESP_HEAD_GET(resp_header, STATUS);
		if (resp_status != 0) {
 ...
 * Return: 0 - success, negative - failure
 **/
static int wait_for_resp_polling(struct hinic5_api_cmd_cell_ctxt *ctxt)
{
	return hinic5_wait_for_timeout(ctxt, check_cmd_resp_handler,
 ...
 * Return: 0 - success, negative - failure
 **/
static int wait_for_api_cmd_completion(struct hinic5_api_cmd_chain *chain,
				       struct hinic5_api_cmd_cell_ctxt *ctxt,
...
static inline void update_api_cmd_ctxt(struct hinic5_api_cmd_chain *chain,
				       struct hinic5_api_cmd_cell_ctxt *ctxt)
{
	ctxt->status = 1;
 ...
 * Return: 0 - success, negative - failure
 **/
static int api_cmd(struct hinic5_api_cmd_chain *chain, u8 node_id,
...
/**
 * hinic5_api_cmd_write - Write API CMD command
...
/**
 * hinic5_api_cmd_read - Read API CMD command
 ...
 * Return: 0 - success, negative - failure
 **/
int hinic5_api_cmd_read(struct hinic5_api_cmd_chain *chain, u8 node_id,
			const void *cmd, u16 size, void *ack, u16 ack_size)
{
	return api_cmd(chain, node_id, cmd, size, ack, ack_size);
}

static enum hinic5_wait_return check_chain_restart_handler(void *priv_data)
{
	struct hinic5_api_cmd_chain *cmd_chain = priv_data;
	u32 reg_addr, val;
 ...
 **/
static int api_cmd_hw_restart(struct hinic5_api_cmd_chain *cmd_chain)
{
	struct hinic5_hwif *hwif = cmd_chain->hwdev->hwif;
	u32 reg_addr, val;
	...
	val |= HINIC5_API_CMD_CHAIN_REQ_SET(1, RESTART);

	hinic5_hwif_write_reg(hwif, reg_addr, val);

	return hinic5_wait_for_timeout(cmd_chain, check_chain_restart_handler,
 ...
 **/
static void api_cmd_ctrl_init(struct hinic5_api_cmd_chain *chain)
{
	struct hinic5_hwif *hwif = chain->hwdev->hwif;
	u32 reg_addr, ctrl;
	u32 size;
	...
	/* Read Modify Write */
	reg_addr = HINIC5_CSR_API_CMD_CHAIN_CTRL_ADDR((u32)chain->chain_type);
	...
	hinic5_hwif_write_reg(hwif, reg_addr, ctrl);
}
 ...
 **/
static void api_cmd_set_status_addr(struct hinic5_api_cmd_chain *chain)
{
	struct hinic5_hwif *hwif = chain->hwdev->hwif;
	u32 addr, val;

	addr = HINIC5_CSR_API_CMD_STATUS_HI_ADDR((u32)chain->chain_type);
	val = upper_32_bits(chain->wb_status_paddr);
	hinic5_hwif_write_reg(hwif, addr, val);

	addr = HINIC5_CSR_API_CMD_STATUS_LO_ADDR((u32)chain->chain_type);
 ...
 **/
static void api_cmd_set_num_cells(struct hinic5_api_cmd_chain *chain)
{
	struct hinic5_hwif *hwif = chain->hwdev->hwif;
	u32 addr, val;
 ...
 **/
static void api_cmd_head_init(struct hinic5_api_cmd_chain *chain)
{
	struct hinic5_hwif *hwif = chain->hwdev->hwif;
	u32 addr, val;

	addr = HINIC5_CSR_API_CMD_CHAIN_HEAD_HI_ADDR((u32)chain->chain_type);
	val = upper_32_bits(chain->head_cell_paddr);
	hinic5_hwif_write_reg(hwif, addr, val);

	addr = HINIC5_CSR_API_CMD_CHAIN_HEAD_LO_ADDR((u32)chain->chain_type);
	val = lower_32_bits(chain->head_cell_paddr);
	hinic5_hwif_write_reg(hwif, addr, val);
}

static enum hinic5_wait_return check_chain_ready_handler(void *priv_data)
{
	struct hinic5_api_cmd_chain *chain = priv_data;
	u32 addr, val;
	u32 hw_cons_idx;
	...
	addr = HINIC5_CSR_API_CMD_STATUS_0_ADDR((u32)chain->chain_type);
	val = hinic5_hwif_read_reg(chain->hwdev->hwif, addr);
	hw_cons_idx = HINIC5_API_CMD_STATUS_GET(val, CONS_IDX);
 ...
 * Return: 0 - success, negative - failure
 **/
static int wait_for_ready_chain(struct hinic5_api_cmd_chain *chain)
{
	return hinic5_wait_for_timeout(chain, check_chain_ready_handler,
				       API_CMD_TIMEOUT, USEC_PER_MSEC);
}
 ...
 **/
static void api_cmd_chain_hw_clean(struct hinic5_api_cmd_chain *chain)
{
	struct hinic5_hwif *hwif = chain->hwdev->hwif;
	u32 addr, ctrl;

	addr = HINIC5_CSR_API_CMD_CHAIN_CTRL_ADDR((u32)chain->chain_type);
	...
	hinic5_hwif_write_reg(hwif, addr, ctrl);
}
 ...
 * Return: 0 - success, negative - failure
 **/
static int api_cmd_chain_hw_init(struct hinic5_api_cmd_chain *chain)
{
 ...
 * Return: 0 - success, negative - failure
 **/
static int alloc_cmd_buf(struct hinic5_api_cmd_chain *chain,
			 struct hinic5_api_cmd_cell *cell, u32 cell_idx)
{
	struct hinic5_api_cmd_cell_ctxt *cell_ctxt = NULL;
	void *dev = chain->hwdev->dev_hdl;
	void *buf_vaddr = NULL;
 ...
 * @cell_idx: the index of the cell
 **/
static void alloc_resp_buf(struct hinic5_api_cmd_chain *chain,
			   struct hinic5_api_cmd_cell *cell, u32 cell_idx)
{
	struct hinic5_api_cmd_cell_ctxt *cell_ctxt = NULL;
	void *resp_vaddr = NULL;
	u64 resp_paddr;
...
static int hinic5_alloc_api_cmd_cell_buf(struct hinic5_api_cmd_chain *chain,
					 u32 cell_idx,
					 struct hinic5_api_cmd_cell *node)
{
	void *dev = chain->hwdev->dev_hdl;
	int err;

	/* For read chain, we should allocate buffer for the response data */
	if (chain->chain_type == HINIC5_API_CMD_MULTI_READ ||
	    chain->chain_type == HINIC5_API_CMD_POLL_READ)
 ...
 * Return: 0 - success, negative - failure
 **/
static int api_cmd_create_cell(struct hinic5_api_cmd_chain *chain, u32 cell_idx,
			       struct hinic5_api_cmd_cell *pre_node,
			       struct hinic5_api_cmd_cell **node_vaddr)
{
	struct hinic5_api_cmd_cell_ctxt *cell_ctxt = NULL;
	struct hinic5_api_cmd_cell *node = NULL;
	void *cell_vaddr = NULL;
	u64 cell_paddr;
	int err;
	...
	err = hinic5_alloc_api_cmd_cell_buf(chain, cell_idx, node);
	if (err != 0)
		return err;
 ...
 * Return: 0 - success, negative - failure
 **/
static int api_cmd_create_cells(struct hinic5_api_cmd_chain *chain)
{
	struct hinic5_api_cmd_cell *node = NULL, *pre_node = NULL;
	void *dev = chain->hwdev->dev_hdl;
	u32 cell_idx;
	int err;
 ...
 * Return: 0 - success, negative - failure
 **/
static int api_chain_init(struct hinic5_api_cmd_chain *chain,
			  struct hinic5_api_cmd_chain_attr *attr)
{
	void *dev = chain->hwdev->dev_hdl;
 ...
 **/
static void api_chain_free(struct hinic5_api_cmd_chain *chain)
{
	void *dev = chain->hwdev->dev_hdl;
 ...
 * Return: 0 - success, negative - failure
 **/
static int api_cmd_create_chain(struct hinic5_api_cmd_chain **cmd_chain,
				struct hinic5_api_cmd_chain_attr *attr)
{
	struct hinic5_hwdev *hwdev = attr->hwdev;
 ...
 **/
static void api_cmd_destroy_chain(struct hinic5_api_cmd_chain *chain)
{
	api_chain_free(chain);
	kfree(chain);
}
...
/**
 * hinic5_api_cmd_init - Initialize all the API CMD chains
			...
			struct hinic5_api_cmd_chain **chain)
{
	...
	if (!COMM_SUPPORT_API_CHAIN(hwdev))
		return 0;
	...
	for (; chain_type < (u8)HINIC5_API_CMD_MAX; chain_type++) {
		...
		if (err != 0) {
			sdk_err(dev, "Failed to create chain %u\n", chain_type);
...
create_chain_err:
	i = HINIC5_API_CMD_WRITE_TO_MGMT_CPU;
	for (; i < chain_type; i++)
