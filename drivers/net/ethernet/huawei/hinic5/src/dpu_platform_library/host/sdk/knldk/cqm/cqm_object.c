 ...
 * File Name     : cqm_object.c
 * Version       : Initial Draft
 * Created       : 2026/5/20
 * Last Modified : 2026/09/16
 * Description   : CQM object management implementation
 */
...
static inline void cqm_object_init(struct tag_cqm_object *object, u32 service_type,
...
static s32 cqm_object_create_check(struct tag_cqm_handle *cqm_handle, u32 service_type)
{
						  ...
						  u32 bitmap_start, u32 bitmap_end)
{
	...
	if (cqm_object_create_check(cqm_handle, service_type) != CQM_SUCCESS) {
		cqm_err(handle->dev_hdl, CQM_FUNCTION_FAIL(cqm_object_create_check));
		return NULL;
	}
	...
	if (CQM_IS_FAKE_PARENT(cqm_handle) &&
		...
		struct tag_cqm_fake_cfg *fake_cfg = &cqm_handle->func_capability.fake_cfg;
		if (fake_cfg->fake_vf_max_pctx == 0) {
			cqm_err(handle->dev_hdl, CQM_WRONG_VALUE(fake_cfg->fake_vf_max_pctx));
			return NULL;
		}
...
static struct tag_cqm_queue *cqm_create_rqs(struct tag_cqm_nonrdma_qinfo *rq_qinfo, struct tag_cqm_handle *cqm_handle,
	struct hinic5_hwdev *handle, u32 init_rq_num)
{
	u32 i;
	/* 3. create queue header */
	rq_qinfo->common.q_header_vaddr = cqm_kmalloc_align(sizeof(struct tag_cqm_queue_header),
	...
	rq_qinfo->common.q_header_paddr = dma_map_single(cqm_handle->dev, rq_qinfo->common.q_header_vaddr,
			   sizeof(struct tag_cqm_queue_header), DMA_BIDIRECTIONAL);
	if (dma_mapping_error(cqm_handle->dev, rq_qinfo->common.q_header_paddr) != 0) {
	...
	for (i = 0; i < init_rq_num; i++) {
		if (cqm_container_create(&rq_qinfo->common.object, NULL, true) != CQM_SUCCESS) {
			cqm_err(handle->dev_hdl, CQM_FUNCTION_FAIL(cqm_container_create));
			goto err2;
		}
...
static bool cqm_object_share_recv_queue_param_check(struct hinic5_hwdev *handle, u32 service_type,
						    enum cqm_object_type object_type,
						    u32 container_size, u32 wqe_size)
{
	...
	if (container_size < wqe_size) {
		cqm_err(handle->dev_hdl, "container_size(0x%x) is smaller than wqe_size(0x%x)!", container_size, wqe_size);
		return false;
	}
 ...
 *   Modification : Created function
 */
struct tag_cqm_queue *cqm5_object_share_recv_queue_create(void *ex_handle, u32 service_type,
							 enum cqm_object_type object_type,
 ...
 */
static bool cqm_object_fc_srq_param_check(struct hinic5_hwdev *handle, u32 service_type,
					       ...
					       void *object_priv)
{
	...
	if (!cqm_object_fc_srq_param_check(handle, service_type, object_type, wqe_size))
		return NULL;
	...
	cqm_object_init(&nonrdma_qinfo->common.object, service_type, object_type,
			wqe_sum, cqm_handle);
...
static bool cqm_object_nonrdma_queue_param_check(struct hinic5_hwdev *handle,
						 enum cqm_object_type object_type, u32 wqe_size)
{
						      ...
						      void *object_priv)
{
	...
	if (!cqm_object_nonrdma_queue_param_check(handle, object_type, wqe_size))
		return NULL;
...
static bool cqm_object_rdma_queue_param_check(struct hinic5_hwdev *handle, u32 service_type,
					      enum cqm_object_type object_type)
{
	/* service_type must be CQM_SERVICE_T_ROCE or CQM_SERVICE_T_UB */
	if (service_type != CQM_SERVICE_T_ROCE && service_type != CQM_SERVICE_T_UB && service_type != CQM_SERVICE_T_VBS) {
		cqm_err(handle->dev_hdl, CQM_WRONG_VALUE(service_type));
		return false;
	}
						     ...
						     u32 index_base, u32 index_number)
{
	...
	/* service_type must be CQM_SERVICE_T_ROCE or CQM_SERVICE_T_UB */
	if (service_type != CQM_SERVICE_T_ROCE && service_type != CQM_SERVICE_T_UB) {
		cqm_err(handle->dev_hdl, CQM_WRONG_VALUE(service_type));
		return NULL;
	}
			    ...
			    struct tag_cqm_cla_table *cla_table)
{
	...
	if (ret == CQM_FAIL) {
		cqm_err(handle->dev_hdl, CQM_FUNCTION_FAIL(cqm_timer_get_func_offset));
		return;
	}
	...
	for (i = func_buf_start; i < func_buf_end; i++) {
		...
		(void)memset(cla_buf->buf_list[i].va, 0,
				cla_buf->buf_size);
	}
	return;
...
void cqm5_function_hash_buf_clear(void *ex_handle, s32 global_funcid)
{
	...
	if (CQM_IS_LB_MODE_1_OR_2(cqm_handle)) {
		...
		cla_table = cqm_cla_table_get(&cqm_handle->bat_table, CQM_BAT_ENTRY_T_HASH);
		loop = 1;
	}
	...
	while (loop > 0) {
		...
		for (i = 0; i < cla_z_buf->buf_number; i++)
			(void)memset(cla_z_buf->buf_list[i].va, 0, cla_z_buf->buf_size);
...
void cqm5_srq_used_rq_container_delete(struct tag_cqm_object *object, u8 *container)
{
	 ...
	 * unmap pa
	 */
	srq_link_wqe = (struct tag_cqm_srq_linkwqe *)((uintptr_t)container + link_wqe_offset);
