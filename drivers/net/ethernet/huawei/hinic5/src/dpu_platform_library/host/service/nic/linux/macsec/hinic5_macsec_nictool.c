			 ...
			 struct himacsec_sa *tar_sa, u8 *sa_cnt, u32 cmd_type)
{
	...
	for (index = 0; index < nic_dev->macsec_res->spec.max_sa; index++) {
		...
		if (cmd_type == MACSEC_TOOL_OP_DUMP) {
			ret = himacsec_cmd_exec_sa_op(nic_dev->lld_dev, &enc_sa.info, MACSEC_CMD_ENC_SA_GET_INFO);
			if (ret != 0) {
				macsec_err(nic_dev->lld_dev->dev, "%s: Exec cmd dump encryption sa config failed, ret=%d", nic_dev->netdev->name, ret);
				return ret;
			}
			enc_sa.info.an = an;
		}
			 ...
			 struct himacsec_sa *tar_sa, u8 *sa_cnt, u32 cmd_type)
{
	...
	for (index = 0; index < nic_dev->macsec_res->spec.max_sa; index++) {
		...
		if (cmd_type == MACSEC_TOOL_OP_DUMP) {
			ret = himacsec_cmd_exec_sa_op(nic_dev->lld_dev, &dec_sa.info, MACSEC_CMD_DEC_SA_GET_INFO);
			if (ret != 0) {
			   ...
			   struct himacsec_cmd_list_sc_buf *cur_sc_buf, u32 cmd_type)
{
	...
	if (cmd_type == MACSEC_TOOL_OP_DUMP) {
		ret = himacsec_cmd_exec_sc_op(nic_dev->lld_dev, &temp_sc->info, MACSEC_CMD_ENC_SC_GET_INFO);
		if (ret != 0) {
			macsec_err(nic_dev->lld_dev->dev, "Dump cmd failed ret=%d, fetch tx sc info fail.", ret);
			return ret;
		}
	}
	ret = himacsec_fill_enc_sa(nic_dev, temp_sc, cur_sc_buf->sc.sa, &cur_sc_buf->sa_cnt, cmd_type);
	if (ret != 0) {
		macsec_err(nic_dev->lld_dev->dev, "Dump cmd failed ret=%d, fetch tx sa info fail.", ret);
		return ret;
	}
			   ...
			   struct himacsec_cmd_list_sc_buf *cur_sc_buf, u32 cmd_type)
{
	...
	if (cmd_type == MACSEC_TOOL_OP_DUMP) {
		ret = himacsec_cmd_exec_sc_op(nic_dev->lld_dev, &temp_sc->info, MACSEC_CMD_DEC_SC_GET_INFO);
		if (ret != 0) {
			macsec_err(nic_dev->lld_dev->dev, "Dump cmd failed ret=%d, fetch rx sc info fail.", ret);
			return ret;
		}
	}
	ret = himacsec_fill_dec_sa(nic_dev, temp_sc, cur_sc_buf->sc.sa, &cur_sc_buf->sa_cnt, cmd_type);
	if (ret != 0) {
		macsec_err(nic_dev->lld_dev->dev, "Dump cmd failed ret=%d, fetch rx sa info fail.", ret);
		return ret;
	}
	cur_sc_buf->sc.info = temp_sc->info;
...
int himacsec_fill_config(struct hinic5_nic_dev *nic_dev, struct himacsec_cmd_list_out *cmd_out, u32 cmd_type)
{
	struct himacsec_cmd_list_sc_buf *enc_sc_buf = (struct himacsec_cmd_list_sc_buf *)cmd_out->enc_sc_buf;
	struct himacsec_cmd_list_sc_buf *dec_sc_buf = (struct himacsec_cmd_list_sc_buf *)cmd_out->dec_sc_buf;
	...
	for (index = 0; index < nic_dev->macsec_res->spec.max_port_sc; index++) {
		// Get encryption direction macsec configuration
		temp_sc = get_g_macsec_port_res(MACSEC_OUTBOUND, port_id);
		if ((temp_sc != NULL) && (MACSEC_SC_STATUS_VALID(temp_sc->status.status.sc) != 0)) {
			/* Calculate address of next receive sc parameter */
			cur_sc_buf = enc_sc_buf + enc_sc_cnt;
			ret = macsec_get_enc_sc_info(nic_dev, temp_sc, cur_sc_buf, cmd_type);
			if (ret != 0) {
				macsec_err(nic_dev->lld_dev->dev, "Dump cmd failed ret=%d, get tx macsec info fail.", ret);
				return ret;
			}
			enc_sc_cnt++;
		}

		// Get decryption direction macsec configuration
		...
		if ((temp_sc != NULL) && (MACSEC_SC_STATUS_VALID(temp_sc->status.status.sc) != 0)) {
			...
			if (ret != 0) {
				macsec_err(nic_dev->lld_dev->dev, "Dump cmd failed ret=%d, get rx macsec info fail.", ret);
				return ret;
			}
			dec_sc_cnt++;
		}
...
int himacsec_fill_mib_sc(struct hinic5_nic_dev *nic_dev, struct himacsec_cmd_mib_out *cmd_out, u64 sci)
{
	...
	if ((enc_sc == NULL) && (dec_sc == NULL)) { // SCI does not exist
...
int himacsec_fill_mib_port(struct hinic5_nic_dev *nic_dev, struct himacsec_cmd_mib_out *cmd_out)
{
	...
	if (ret != 0) {
		macsec_err(nic_dev->lld_dev->dev, "%s: Exec cmd query port mib failed, ret=%d", nic_dev->netdev->name, ret);
	}
...
int macsec_cmd_list(struct hinic5_nic_dev *nic_dev, const void *buf_in, u32 in_size, void *buf_out, u32 *out_size)
{
	...
	if ((himacsec_cmd_check_param_buf(buf_in, in_size, sizeof(struct himacsec_cmd_hdr)) != 0) ||
		(himacsec_cmd_check_param_buf(buf_out, *out_size, sizeof(struct himacsec_cmd_list_out)) != 0)) {
		return -EINVAL;
	}
	...
	if (ret != 0) {
		macsec_err(nic_dev->lld_dev->dev, "Cmd list/dump macsec config exec failed, ret=%d", ret);
	}
...
int macsec_cmd_mib(struct hinic5_nic_dev *nic_dev, const void *buf_in, u32 in_size, void *buf_out, u32 *out_size)
{
	...
	if ((himacsec_cmd_check_param_buf(buf_in, in_size, exp_in_size) != 0) ||
		(himacsec_cmd_check_param_buf(buf_out, *out_size, sizeof(struct himacsec_cmd_mib_out)) != 0)) {
		return -EINVAL;
	}
...
int macsec_cmd_flush(struct hinic5_nic_dev *nic_dev, const void *buf_in, u32 in_size, void *buf_out, u32 *out_size)
{
	...
	macsec_info(nic_dev->lld_dev->dev, "MACsec flush process, obj_type=0x%x", cmd_in->hdr.obj_type);
	...
	for (index = 0; index < macsec_res->spec.max_port_sc; index++) {
		(void)memset(enc_sc, 0, sizeof(struct himacsec_sc));
		(void)memset(dec_sc, 0, sizeof(struct himacsec_sc));
	}
	return ret;
}

int himacsec_nictool_add_sc(struct hinic5_nic_dev *nic_dev, const struct himacsec_cmd_in *cmd_in, u32 in_size,
...
int himacsec_nictool_add_sa(struct hinic5_nic_dev *nic_dev, const struct himacsec_cmd_in *cmd_in, u32 in_size,
	crypt_direction_e direct)
{
	...
	if (in_size != exp_in_size) {
		macsec_err(nic_dev->lld_dev->dev, "Add sa failed, size error, in size=0x%x, exp_size=0x%x", in_size, exp_in_size);
		return -EINVAL;
	}

	// SM4 algorithm needs to verify whether feature negotiation result is supported
	if ((sa->current_crypto_algo == HIMACSEC_CRYPTO_ALGO_SM4) &&
		(macsec_res->himacsec_feature[0] & (u64)MACSEC_F_SUPPORT_SM4) == 0) {
		macsec_err(nic_dev->lld_dev->dev, "Add sa failed, unsupported algorithm(0x%x)", sa->current_crypto_algo);
		return -EINVAL;
	}

	// Need to explicitly configure the next_pn table on the RX side
...
int himacsec_cmd_set_enc_sc(struct hinic5_nic_dev *nic_dev, struct himacsec_cmd_in *cmd_in, u32 in_size)
{
	...
	if (priv_sc_ptr == NULL) {
		macsec_err(nic_dev->lld_dev->dev, "%s: Set encryption sc failed, sc not found", nic_dev->netdev->name);
		return -EINVAL;
	}

	(void)memcpy(&enc_sc, &priv_sc_ptr->info, sizeof(macsec_sc_info_s));
	// Use sc copy data; after update completes, driver internal data is automatically updated
	if ((param->set_flag_bitmap & HIMACSEC_SET_SC_ENCODING_SA_BIT_VAL) != 0) {
		// sa validity check
		if (himacsec_get_valid_dev_sa(nic_dev, param->sci, param->sc.encoding_sa, MACSEC_OUTBOUND) == NULL) {
			macsec_err(nic_dev->lld_dev->dev, "%s: Update encodingsa failed, sa '%d' invalid",
				nic_dev->netdev->name, param->sc.encoding_sa);
			return -EINVAL;
		}
		enc_sc.encoding_sa = param->sc.encoding_sa % macsec_res->spec.max_sa; // an converted to sa_index
	}
...
int himacsec_cmd_set_dec_sc(struct hinic5_nic_dev *nic_dev, struct himacsec_cmd_in *cmd_in, u32 in_size)
{
	struct himacsec_sc *priv_sc_ptr = NULL;
	struct himacsec_cmd_set_sc_in *param = NULL;
	u32 exp_in_size = sizeof(struct himacsec_cmd_set_sc_in) + sizeof(struct himacsec_cmd_hdr);
	...
	if (priv_sc_ptr == NULL) {
		macsec_err(nic_dev->lld_dev->dev, "%s: Set decryption sc failed, sc not found", nic_dev->netdev->name);
		return -EINVAL;
	}
	(void)memcpy(&dec_sc, &priv_sc_ptr->info, sizeof(macsec_sc_info_s));
