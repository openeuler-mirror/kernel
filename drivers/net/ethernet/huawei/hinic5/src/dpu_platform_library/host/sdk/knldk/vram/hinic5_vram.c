					...
					void *data)
{
	...
	if (g_hiudk_async_ctrl.flush_infos[i].lld_dev) {
		async_schedule_domain(hinic5_flush_dev, &g_hiudk_async_ctrl.flush_infos[i], &g_hiudk_async_domain);
	}
	}
...
int wait5_for_devices_flush(struct notifier_block *nb,
					unsigned long action,
					void *data)
{
	int i;
