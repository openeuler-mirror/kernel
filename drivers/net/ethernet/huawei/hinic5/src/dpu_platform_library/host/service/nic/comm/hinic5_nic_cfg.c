 ...
 * Description   : hinic5 nic configuration implementation
 */
...
int hinic5_set_sq_ci_ctx(struct hinic5_nic_io *nic_io, struct hinic5_sq_attr *attr)
{
	...
	(void)memset(&cons_idx_attr, 0, sizeof(cons_idx_attr));
	...
	err = l2nic_msg_to_mgmt_sync(nic_io->hwdev, HINIC5_NIC_CMD_SQ_CI_ATTR_SET,
...
int hinic5_set_rq_ci_ctx(struct hinic5_nic_io *nic_io, struct hinic5_rq_attr *attr)
{
	...
	(void)memset(&cons_idx_ctx, 0, sizeof(cons_idx_ctx));
	...
	err = l2nic_msg_to_mgmt_sync(nic_io->hwdev, HINIC5_NIC_CMD_SET_RQ_CI_CTX,
...
int hinic5_get_default_mac(void *hwdev, u8 *mac_addr)
{
	...
	err = l2nic_msg_to_mgmt_sync(hwdev, HINIC5_NIC_CMD_GET_MAC,
				     &mac_info, sizeof(mac_info),
		&mac_info, &out_size);
			      ...
			      u16 vlan_id, u16 func_id)
{
	...
	(void)memset(&vlan_info, 0, sizeof(vlan_info));
...
int hinic5_set_vport_enable(void *hwdev, u16 func_id, bool enable, u16 channel)
{
	...
	en_state.rx_compact_wqe_en = (hinic5_get_rq_wqe_type(nic_io->hwdev) == HINIC5_COMPACT_RQ_WQE);
...
int hinic5_get_dscp_by_cos(void *hwdev, u8 cos, u8 *dscp)
{
	...
	if (nic_io->dcb_state.dcb_on != 0 && nic_io->dcb_state.trust == HINIC5_DCB_DSCP) {
		*dscp = nic_io->cos2dscp[cos];
		return 0;
	}

	return -EINVAL;
}
EXPORT_SYMBOL(hinic5_get_dscp_by_cos);

static void hinic5_dcb_build_cos2dscp(const u8 *dscp2cos, u8 *cos2dscp)
{
	u8 cos, dscp;

	/* Descending traversal: dscp overwrites from large to small, ultimately keeping the dscp with the smallest index (first mapping) */
	for (dscp = NIC_DCB_IP_PRI_MAX; dscp > 0; ) {
		dscp--;
		cos = dscp2cos[dscp];
		if (cos < NIC_DCB_UP_MAX)
			cos2dscp[cos] = dscp;
			  ...
			  struct hinic5_dcb_state *dcb_state)
{
	...
	hinic5_dcb_build_cos2dscp(nic_io->dcb_state.dscp2cos, nic_io->cos2dscp);
...
int hinic5_get_pf_dcb_state(void *hwdev, struct hinic5_dcb_state *dcb_state)
{
	...
	(void)memset(&vf_dcb, 0, sizeof(vf_dcb));
			       ...
			       struct nic_pause_config *nic_pause)
{
	...
	(void)memset(&pause_info, 0, sizeof(pause_info));
...
int hinic5_sync_dcb_state(void *hwdev, u8 op_code, u8 state)
{
	...
	(void)memset(&dcb_state, 0, sizeof(dcb_state));
...
int hinic5_flush_qps_res(void *hwdev)
{
	...
	(void)memset(&sq_res, 0, sizeof(sq_res));
...
int hinic5_cache_out_qps_res(void *hwdev)
{
	...
	(void)memset(&qp_res, 0, sizeof(qp_res));
...
static int hinic5_get_vport_stats_by_cmdq(void *hwdev, u16 func_id, struct hinic5_vport_stats *stats)
{
...
static int hinic5_get_vport_stats_by_mailbox(void *hwdev, u16 func_id, struct hinic5_vport_stats *stats)
{
...
static int nic_feature_nego(void *hwdev, u8 opcode, u64 *s_feature, u16 size)
{
	...
	if ((hwdev == NULL) || (s_feature == NULL) || (size > NIC_MAX_FEATURE_QWORD))
		return -EINVAL;
...
static int hinic5_get_bios_pf_bw_limit(void *hwdev, u32 *pf_bw_limit)
{
	...
	err = l2nic_msg_to_mgmt_sync(hwdev, HINIC5_NIC_CMD_BIOS_CFG, &cfg, sizeof(cfg),
				     &cfg, &out_size);
...
int hinic5_set_pf_rate(void *hwdev, u8 speed_level)
{
	...
	err = l2nic_msg_to_mgmt_sync(hwdev, HINIC5_NIC_CMD_SET_MAX_MIN_RATE, &rate_cfg,
...
int hinic5_force_drop_tx_pkt(void *hwdev)
{
	...
	(void)memset(&pkt_drop, 0, sizeof(pkt_drop));
...
int hinic5_set_rx_mode(void *hwdev, u32 enable)
{
	...
	(void)memset(&rx_mode_cfg, 0, sizeof(rx_mode_cfg));
...
int hinic5_set_rx_vlan_offload(void *hwdev, u8 en)
{
	...
	(void)memset(&vlan_cfg, 0, sizeof(vlan_cfg));
...
int hinic5_update_mac_vlan(void *hwdev, u16 old_vlan, u16 new_vlan, int vf_id)
{
	...
	if ((nic_io->vf_infos == NULL) || is_zero_ether_addr(vf_info->drv_mac_addr))
		return 0;
			     ...
			     u8 lro_max_pkt_len)
{
	...
	err = l2nic_msg_to_mgmt_sync(hwdev, HINIC5_NIC_CMD_CFG_RX_LRO,
...
static int hinic5_set_rx_lro_timer(void *hwdev, u32 timer_value)
{
	...
	(void)memset(&lro_timer, 0, sizeof(lro_timer));
...
EXPORT_SYMBOL(hinic5_set_veb_offload);
...
int hinic5_set_vlan_fliter(void *hwdev, u32 vlan_filter_ctrl)
{
	...
	(void)memset(&vlan_filter, 0, sizeof(vlan_filter));
...
int hinic5_add_tcam_rule(void *hwdev, struct nic_tcam_cfg_rule *tcam_rule)
{
	...
	(void)memset(&tcam_cmd, 0, sizeof(struct nic_cmd_fdir_add_rule));
	memcpy(
			(void *)&tcam_cmd.rule, (void *)tcam_rule, sizeof(struct nic_tcam_cfg_rule));
...
int hinic5_del_tcam_rule(void *hwdev, u32 index)
{
	...
	(void)memset(&tcam_cmd, 0, sizeof(struct nic_cmd_fdir_del_rules));
...
int hinic5_set_fdir_tcam_rule_filter(void *hwdev, bool enable)
{
	...
	(void)memset(&port_tcam_cmd, 0, sizeof(port_tcam_cmd));
...
int hinic5_flush_tcam_rule(void *hwdev)
{
	...
	if (nic_io == NULL)
		return -EINVAL;
	(void)memset(&tcam_flush, 0, sizeof(struct nic_cmd_flush_tcam_rules));
...
int hinic5_del_tc_flow_rule(void *hwdev, u16 rule_id)
{
	...
	(void)memset(&cmd, 0, sizeof(struct hinic5_tc_cfg_info));
	cmd.index = rule_id;
...
int hinic5_flush_tc_flow_rule(void *hwdev, ulong *bitmap)
{
	...
	err = l2nic_msg_to_mgmt_sync(hwdev, HINIC5_NIC_CMD_FLUSH_TC_FLOW,
					 &cmd, sizeof(cmd),
					 &cmd, &out_size);
	if ((err != 0) || (cmd.head.status != 0) || (out_size == 0)) {
		nic_err(nic_io->dev_hdl,
...
int hinic5_get_pfe_cfg(void *hwdev, struct hinic5_tc_pfe_cfg_reg_info *cfg_info)
{
	...
	(void)memset(cfg_info, 0, out_size);
...
int hinic5_send_arp_to_mpu(void *hwdev, struct hinic5_arp_pkt_info *info)
{
	...
	err = hinic5_msg_to_mgmt_sync(hwdev, HINIC5_MOD_CFM, CFM_MPU_CMD_PASS_ARP_PKT, info,
		sizeof(struct hinic5_arp_pkt_info), info, &out_size, HINIC5_BOND_MSG_TIMEOUT_MS, HINIC5_CHANNEL_NIC);
...
bool hinic5_check_dev_need_dual_send(void *hwdev)
{
	...
	if ((nic_io->feature_cap & NIC_F_ARP_DUAL) != 0 && (nic_io->feature_cap & NIC_F_HALF_BOND_OFFLOAD) != 0)
		return true;
...
int hinic5_vxlan_port_config(void *hwdev, u16 func_id, u16 port, u8 action, u8 pkt_fmt)
{
	...
	if ((err != 0) || (out_size == 0) || (vxlan_port_info.msg_head.status != 0)) {
		if (vxlan_port_info.msg_head.status == HINIC5_VXLAN_DPORT_SET_UNSUPPORT && err == 0) {
			return -EOPNOTSUPP;
		}
		if (vxlan_port_info.msg_head.status == HINIC5_VXLAN_DPORT_SET_BY_HINICADM && err == 0) { // other tool set failed
			nic_warn(nic_io->dev_hdl,
				"Dst port has already been set\n");
		} else {
			nic_err(nic_io->dev_hdl,
				"Failed to %s vxlan dst port, cmd: 0x%x, err: %d, status: 0x%x, out size: 0x%x\n",
				action == HINIC5_CMD_OP_ADD ? "add" : "delete", HINIC5_NIC_CMD_CFG_VXLAN_PORT,
