 ...
 * File Name     : cqm_bitmap_table.c
 * Version       : Initial Draft
 * Created       : 2026/5/20
 * Last Modified : 2026/09/16
 * Description   : CQM bitmap table implementation
 */
...
struct malloc_memory {
	bool (*check_alloc_mode)(const struct hinic5_hwdev *handle, const struct tag_cqm_buf *buf);
	s32 (*malloc_func)(struct hinic5_hwdev *handle, struct tag_cqm_buf *buf);
};
...
static bool check_use_vram(const struct hinic5_hwdev *handle, const struct tag_cqm_buf *buf)
{
	return buf->buf_info.use_vram != 0 ? true : false;
}

static bool check_use_non_vram(const struct hinic5_hwdev *handle, const struct tag_cqm_buf *buf)
{
	return buf->buf_info.use_vram != 0 ? false : true;
}

static bool check_for_use_node_alloc(const struct hinic5_hwdev *handle, const struct tag_cqm_buf *buf)
{
...
static bool check_for_nouse_node_alloc(const struct hinic5_hwdev *handle, const struct tag_cqm_buf *buf)
{
	if (buf->buf_info.use_vram == 0 && handle->board_info.service_mode != 0)
		return true;
...
static u8 cqm_vram_node(struct hinic5_hwdev *handle)
{
	if (nr_node_ids > 0) {
		...
		// nr_node_ids indicates the maximum number of available NUMA nodes, which will not exceed the u8 maximum value
...
static s32 cqm_buf_vram_kalloc(struct hinic5_hwdev *handle, struct tag_cqm_buf *buf)
{
	void *vaddr = NULL;
	u32 i;
...
static s32 cqm_buf_use_node_alloc_page(struct hinic5_hwdev *handle, struct tag_cqm_buf *buf)
{
...
static s32 cqm_buf_unused_node_alloc_page(struct hinic5_hwdev *handle, struct tag_cqm_buf *buf)
{
	gfp_t flags = GFP_KERNEL | __GFP_ZERO;
	u32 order, i;
	void *va = NULL;
...
s32 cqm_buf_list_alloc(struct tag_cqm_buf *buf)
{
	...
	(void)memset(buf->buf_list, 0, size);
	return CQM_SUCCESS;
}
...
s32 cqm_buf_alloc(struct tag_cqm_handle *cqm_handle, struct tag_cqm_buf *buf, bool direct)
{
	...
	if (unlikely(ret != CQM_SUCCESS)) {
		cqm_warn(handle->dev_hdl, CQM_FUNCTION_FAIL(linux_cqm_buf_alloc_page));
		goto err1;
	}
...
err3:
	for (i = 0; i < buf->buf_number; i++)
		dma_unmap_single(cqm_handle->dev, buf->buf_list[i].pa, buf->buf_size,
				 DMA_BIDIRECTIONAL);
err2:
	...
	for (i = 0; i < buf->huge_buf_number; i++) {
		if ((i + 1) == buf->huge_buf_number)
			bufs_addr[i].huge_buf_size =
			    PAGE_SIZE <<
			    get_order(total_size - CQM_HUGE_BUF_SIZE * i);
		else
...
s32 cqm_buf_alloc(struct tag_cqm_handle *cqm_handle, struct tag_cqm_buf *buf, bool direct)
{
	...
	if (buf->buf_size > CQM_HUGE_BUF_SIZE) {
		cqm_err(handle->dev_hdl, "Buffer size(0x%x) is large than huge buffer size(0x%x)\n",
	...
	(void)memset(buf->buf_list, 0,
		buf->buf_number * sizeof(struct tag_cqm_buf_list));

	/* Page for applying for each buffer */
	if (cqm_buf_alloc_page(cqm_handle, buf) == CQM_FAIL) {
		cqm_err(handle->dev_hdl, CQM_FUNCTION_FAIL(win_cqm_buf_alloc_page));
		goto err1;
	}

	/* PCI mapping of the buffer */
	if (cqm_buf_alloc_map(cqm_handle, buf) == CQM_FAIL) {
		cqm_err(handle->dev_hdl, CQM_FUNCTION_FAIL(win_cqm_buf_alloc_map));
		goto err2;
	}
				     ...
				     cqm_cla_cache_invalid_cmd_s *cmd_info)
{
	...
	cqm_handle->cmdq_ops->prepare_cmd_cache_invalidate(cmd_info, buf_in, &cmd);
...
s32 cqm_cla_cache_invalid(struct tag_cqm_handle *cqm_handle, dma_addr_t pa, u32 cache_size)
{
	...
	(void)memcpy(&cla_gpa_h, &gpa, sizeof(u32));
...
static s32 cqm_single_bitmap_init(struct tag_cqm_bitmap *bitmap)
{
	...
	if (bitmap->bitmap_info.use_vram != 0)
		bitmap->table = hi5_vram_kalloc(bitmap->bitmap_info.buf_vram_name, nbytes);
	else
	...
	(void)memset(bitmap->table, 0, nbytes);
...
static s32 cqm_bitmap_init_by_type(u32 type, struct tag_cqm_bitmap *bitmap, struct tag_cqm_func_capability *capability)
{
	switch (type) {
...
s32 cqm_bitmap_init(struct tag_cqm_handle *cqm_handle)
{
	...
	for (i = 0; i < CQM_BAT_ENTRY_MAX; i++) {
		...
		err = snprintf(bitmap->bitmap_info.buf_vram_name,
				 VRAM_NAME_MAX_LEN,
				 "%s%s%02u", cla_table->name,
				 VRAM_CQM_BITMAP_BASE, cla_table->type);
		if (err < 0) {
			cqm_err(handle->dev_hdl, "cqm bitmap vram name snprintf failed");
			return CQM_FAIL;
		}

		if (cqm_bitmap_init_by_type(cla_table->type, bitmap, capability) == CQM_SUCCESS) {
					      ...
					      u32 start, u32 end, u32 xid)
{
	...
	if (start <= end)
		index = cqm_bitmap_find_with_lowbits_forward(bitmap, start, end,
							     lowbits, lowbits_mask);
	else
		index = cqm_bitmap_find_with_lowbits_reverse(bitmap, start, end,
							     lowbits, lowbits_mask);
...
u32 cqm_bitmap_alloc_lowbits_align(struct tag_cqm_bitmap *bitmap,
				   struct tag_cqm_bitmap_range *bp_range,
				   struct tag_cqm_handle *cqm_handle, u32 xid, bool update_last)
{
	struct hinic5_hwdev *handle = cqm_handle->ex_handle;
	ulong *table = bitmap->table;
	...
	if (search_mode == CQM_XID_SEARCH_ALL) {
		last = bitmap->last;
		/* Search for an idle bit from the last position. */
		index = cqm_bitmap_find_with_lowbits_align(bitmap, last, max_num, xid);
		...
		index = cqm_bitmap_find_with_lowbits_align(bitmap, bp_range->start, bp_range->end,
							   xid);
	}
...
static inline void bitmap_set_table(struct tag_cqm_bitmap *bitmap, ulong *table, u32 *ret_index, u32 index)
{
	spin_lock(&bitmap->lock);
...
static s32 cqm_single_object_table_init(struct tag_cqm_object_table *obj_table)
{
	...
	(void)memset(obj_table->table, 0, obj_table->max_num * sizeof(void *));
	return CQM_SUCCESS;
}
