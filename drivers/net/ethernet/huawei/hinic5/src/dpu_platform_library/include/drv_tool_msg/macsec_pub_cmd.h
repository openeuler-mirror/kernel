...
#define HIMACSEC_MAX_SA_IN_SC 4 // The macsec protocol stipulates that SC can have a maximum of 4 SAs
...
#define HIMACSEC_CRYPTO_ALGO_AES 0 // TODO unify algorithm types
...
#define HIMACSEC_XPN_MAX_REPLAY_WINDOWS ((1 << 30) - 1) // Maximum replay window in xpn mode is 2^30 - 1
...
// mib query type
typedef enum {
	HIMACSEC_TOOL_MIB_TYPE_SC = 0,  // SC MIB info
	HIMACSEC_TOOL_MIB_TYPE_PORT,    // PORT MIB info
	HIMACSEC_TOOL_MIB_TYPE_MAX
} himacsec_tool_mib_type_e;

// Encryption direction sc protection mode
typedef enum {
	PROTECTION_MODE_INTERITY_ONLY = 0,      // Integrity check only
	PROTECTION_MODE_CONFIDENTIALITY,        // Integrity check and encryption
	PROTECTION_MODE_OFFSET_CONFIDENTIALITY, // Integrity check and encryption, encrypted data can be offset by 0/30/50B
	PROTECTION_MODE_MAX
} himacsec_protection_mode_e;

// Decryption direction sc validation mode
typedef enum {
	VALIDATE_MODE_DISABLE = 1,  // Do not validate packets
	VALIDATE_MODE_CHECK,        // Validate only, do not filter
...
// Supported encryption offset specification
typedef enum {
...
// macsec object operation object
typedef enum {
...
// The command's inbuf contains a hdr
