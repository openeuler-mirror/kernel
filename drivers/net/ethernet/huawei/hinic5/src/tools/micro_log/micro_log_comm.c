...
int hinic5_set_microlog_cmdq(void *hwdev, void *microlog_msg, size_t msg_len, u8 cmd)
{
	...
	err = hinic5_cmdq_direct_resp(hwdev, HINIC5_MOD_COMM, cmd, cmd_buf, &out_param, 0, HINIC5_CHANNEL_NIC);
	if ((err) || (out_param)) {
...
int hinic5_microlog_gpa_set(void *hwdev, u64 p_addr, u8 lt_index)
{
	...
	return hinic5_set_microlog_cmdq(hwdev, (void *)&microlog_gpa, sizeof(cmdq_microlog_gpa_set_s), COMM_CMD_MICROLOG_GPA_SET);
}
