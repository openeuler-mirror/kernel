...
static int micro_log_read_fw_cfg_info(void *hwdev, fw_info_s *cfg_info)
{
	...
	(void)memset(&query_fw_input, 0, sizeof(struct cmd_query_fw));
	(void)memset(&query_fw_output, 0, sizeof(struct cmd_fw_info));
	...
	ret = hinic5_msg_to_mgmt_sync(hwdev, HINIC5_MOD_COMM, COMM_MGMT_CMD_QUERY_FW_INFO, &query_fw_input,
		sizeof(struct cmd_query_fw), &query_fw_output, &out_size, 0, HINIC5_CHANNEL_COMM);
...
static int micro_log_read_sim_data(void *hwdev, struct nic_log_info *simdata_info, u32 read_offset, u32 sim_data_len)
{
	u32 ret;
	...
	(void)memset(simdata_info, 0, sizeof(struct nic_log_info));
	(void)memset(&simdata_buf_in, 0, sizeof(struct nic_log_info));
	simdata_buf_in.offset = read_offset;
	simdata_buf_in.log_or_index = NPU_COMM_GET_SIM_DATA;
	/* Considering compatibility between new and old hinic tools, use data[3:0] field to replace file_size, occupying 32bit */
	*(u32 *)(void *)simdata_buf_in.data = sim_data_len;

	ret = hinic5_msg_to_mgmt_sync(hwdev, HINIC5_MOD_COMM, COMM_MGMT_CMD_GET_LOG, &simdata_buf_in,
	    sizeof(struct nic_log_info), simdata_info, &out_size, 0, HINIC5_CHANNEL_COMM);
...
int mirco_log_get_sim_data_from_flash(void *hwdev, struct micro_log_info *log_info)
{
	...
	log_info->micro_log_data_addr = (char *)kzalloc((sim_data_len + 1), GFP_KERNEL);
	if (log_info->micro_log_data_addr == NULL) {
		microlog_err("malloc micro_log_data_addr fail.");
		return -EFAULT;
	}

	/* mbox reads at most 1K data each time */
