 ...
 * Description   : Used to dynamically change host log status: supports dynamic switching between info logs and err logs
 */
...
#define LOG_LEVEL_MAX_LEN 2 /* Normal case: only two characters used, storing the level character and the newline from echo */
#define DEC_CODE 10
 ...
 * 3) state = 2, print err + warn logs
 * 4) state = 3, print err + warn + info logs
 */
static int micro_log_state_set(void *hwdev, enum log_level_type state)
{
	...
	err = hinic5_microlog_ctrl_info_set(hwdev, log_info->nic_micro_log_enable, log_info->all_ci, state);
	if (err) {
...
static ssize_t log_level_proc_write(struct file *file, const char __user *buff, size_t len, loff_t *off)
{
	ssize_t wr_len;
	...
	microlog_info("procfile write: %c, %s, wr_len:0x%x", log_level_buffer[0], log_level_buffer, (u32)wr_len);
	...
	(void)snprintf(log_level_procfs_buffer_rd, PROCFS_RD_BUFFER_SIZE, "%s\n", log_level_msg[num].level_info);
	return wr_len;
}
...
static ssize_t log_level_proc_read(struct file *file, char __user *buff, size_t len, loff_t *off)
{
	...
	if (*off >= plen) {
		/* If all data has been read, return 0, otherwise it will keep printing in a loop */
		return 0;
	}
...
static int check_params_valid(void *hwdev)
{
	...
	(void)strncpy(hinic_dev_name, log_info->hinic_micro_log_task.name, HINIC_DEV_NAME_LEN);
...
int micro_log_procfs_init(void *hwdev)
{
	...
	proc_log_level_file = proc_create(LOG_LEVEL_PROCFS_NAME, 0644, proc_hinic_dev_dir, &log_level_proc_fops);
