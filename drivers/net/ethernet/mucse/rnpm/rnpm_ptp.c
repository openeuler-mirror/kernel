...
static int init_systime(void __iomem *ioaddr, u8 port, u32 sec, u32 nsec)
{
	...
	//printk(KERN_DEBUG "init systime: %x\n", sec);
...
int rnpm_ptp_setup_ptp(struct rnpm_adapter *pf, u32 value)
{
	...
	pf->hwts_ops->init_systime(pf->hw.hw_addr, port,
...
int rnpm_ptp_set_ts_config(struct rnpm_adapter *pf, struct ifreq *ifr)
{
	...
	netdev_info(pf->netdev, "%s config flags:0x%x, tx_type:0x%x, rx_filter:0x%x\n",
	...
	netdev_info(pf->netdev, "ptp config rx filter 0x%.2x tx_type 0x%.2x rx_en[%d] tx_en[%d]\n",
			config.rx_filter,
			config.tx_type,
			pf->ptp_rx_en,
			pf->ptp_tx_en);
	if (!pf->ptp_rx_en && !pf->ptp_tx_en)
		...
		value = (RNPM_PTP_TCR_TSENA | RNPM_PTP_TCR_TSCFUPDT |
				RNPM_PTP_TCR_TSCTRLSSR | tstamp_all |
...
{
	struct rnpm_adapter *adapter = container_of(work, struct rnpm_adapter,
			tx_hwtstamp_work);
	...
	if (rnpm_rd_reg(ioaddr + RNPM_ETH_PTP_TX_TSVALUE_STATUS(port)) & 0x01) {
		 ...
		 * from high to low
		 */
		//printk(KERN_DEBUG "port %d call clean ptp %llx %llx\n", port, nanosec, sec);
		...
		struct sk_buff *skb)
{
	...
	netdev_dbg(adapter->netdev, "ptp get hardware ts-sec %llu ts-nanosec %llu\n",
			tsvalueh, tsvaluel);
