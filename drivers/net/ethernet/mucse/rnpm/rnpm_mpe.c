 ...
 * @rpu_bin : optional
 */
static int rnpm_download_and_start_rpu(struct rnpm_pf_adapter *pf_adapter,
	struct rnpm_hw *hw, char *rpu_base, const unsigned int *mpe_bin,
	const int mpe_bin_sz, const unsigned int *mpe_data, const int mpe_data_sz,
...
int rnpm_rpu_mpe_start(struct rnpm_pf_adapter *pf_adapter)
{
	...
	if (pf_adapter->hw.pfvfnum == PF_NUM(0)) {
		err = rnpm_download_and_start_rpu(pf_adapter, hw, hw->rpu_addr,
			    ...
			    int queue_end)
{
	...
	for (nr_mpe = 0; nr_mpe < 4; nr_mpe++) {
		if (BIT(nr_mpe) & mpe_id_bitmask) {
			...
			rnpm_mpe_shm_write32(pf_adapter, nr_mpe, offset, 0);

			// set new value
			offset = irss_off + lane * sizeof(struct rss_queue_info);
			offset += offsetof(struct rss_queue_info, queue_start);
			rnpm_mpe_shm_write32(pf_adapter, nr_mpe, offset, queue_start);

			offset = irss_off + lane * sizeof(struct rss_queue_info);
			offset += offsetof(struct rss_queue_info, queue_end);
			rnpm_mpe_shm_write32(pf_adapter, nr_mpe, offset, queue_end);

			offset = irss_off + lane * sizeof(struct rss_queue_info);
			offset += offsetof(struct rss_queue_info, valid);
			rnpm_mpe_shm_write32(pf_adapter, nr_mpe, offset, INFO_VALID);

			// set nic-mode to DCCM
			offset = offsetof(struct mpe_shm_comm, nic_mode);
			rnpm_mpe_shm_write32(pf_adapter, nr_mpe, offset, pf_adapter->hw.mode);

			offset = offsetof(struct mpe_shm_comm, valid_magic);
			rnpm_mpe_shm_write32(pf_adapter, nr_mpe, offset, DRV_MPE_SHM_MAGIC);
		}
	}
