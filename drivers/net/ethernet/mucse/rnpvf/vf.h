...
struct rnp_mac_operations {
	s32 (*init_hw)(struct rnpvf_hw *hw);
	s32 (*reset_hw)(struct rnpvf_hw *hw);
	s32 (*start_hw)(struct rnpvf_hw *hw);
	s32 (*clear_hw_cntrs)(struct rnpvf_hw *hw);
	enum rnp_media_type (*get_media_type)(struct rnpvf_hw *hw);
	u32 (*get_supported_physical_layer)(struct rnpvf_hw *hw);
	s32 (*get_mac_addr)(struct rnpvf_hw *hw, u8 *mac);
	s32 (*get_queues)(struct rnpvf_hw *hw);
	s32 (*stop_adapter)(struct rnpvf_hw *hw);
	s32 (*get_bus_info)(struct rnpvf_hw *hw);
	int (*read_eth_reg)(struct rnpvf_hw *hw, int reg, u32 *value);

	int (*get_mtu)(struct rnpvf_hw *hw);
	int (*set_mtu)(struct rnpvf_hw *hw, int mtu);
	int (*req_reset_pf)(struct rnpvf_hw *hw);

	/* Link */
	s32 (*setup_link)(struct rnpvf_hw *hw, rnp_link_speed speed,
			  bool autoneg, bool autoneg_wait_to_complete);
	s32 (*check_link)(struct rnpvf_hw *hw, rnp_link_speed *speed,
			  bool *autoneg, bool autoneg_wait_to_complete);
	s32 (*get_link_capabilities)(struct rnpvf_hw *hw, rnp_link_speed *speed,
				     bool *auneg);
...
struct rnpvf_info {
	...
	s32 (*get_invariants)(struct rnpvf_hw *hw);
};
