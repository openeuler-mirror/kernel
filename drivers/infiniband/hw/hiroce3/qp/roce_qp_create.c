	...
	struct roce3_qp *rqp, struct create_qp_cmd *ucmd)
{
	...
	if (rqp->qp_type != IB_QPT_XRC_TGT) {
		/* between max and min */
		/*lint -e587*/
		tmp_max_sq_wqe_sz = (u32)(ROCE_ROUNDUP_POW_OF_TWO(
			rdev->rdma_cap.max_sq_desc_sz) & 0xffffffff);
		/*lint +e587*/
	...
	struct roce3_qp *rqp, struct roce3_device *rdev)
{
	...
	if (ret != 0) {
		dev_err(rdev->hwdev_hdl,
			"[ROCE, ERR] %s: Failed to create qp gsi, func_id(%u)\n",
			__func__, rdev->glb_func_id);
		return ret;
	}
...
int roce3_create_qp_common(struct ib_pd *ibpd, struct ib_qp_init_attr *init_attr,
	struct ib_udata *udata, struct roce3_qp *rqp, int qp_ext)
{
	int ret = 0;
...
int roce3_create_qp(struct ib_qp *qp, struct ib_qp_init_attr *qp_init_attr,
	struct ib_udata *udata)
{
