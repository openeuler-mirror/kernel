	...
	struct ubcore_tpg_cfg *tpg_cfg, struct ubcore_cmd_tp_cfg *tp_cfg_arg)
{
	...
	if (tpg_cfg->tp_cnt > UBCORE_MAX_TP_CNT_IN_GRP ||
...
static int ubcore_cmd_set_upi(struct ubcore_cmd_hdr *hdr)
{
	...
	for (i = 0; i < dev->attr.max_eid_cnt; i++) {
		...
		if (pattern3_upi == UCBORE_INVALID_UPI) {
			cfg.upi = arg.in.upi;
			(void)ubcore_add_ueid(
				dev, (uint16_t)UBCORE_NON_VIRTUALIZATION_FE_IDX, &cfg);
		} else {
			cfg.upi = pattern3_upi;
			(void)ubcore_delete_ueid(
				dev, (uint16_t)UBCORE_NON_VIRTUALIZATION_FE_IDX, &cfg);
			cfg.upi = arg.in.upi;
			(void)ubcore_add_ueid(
				dev, (uint16_t)UBCORE_NON_VIRTUALIZATION_FE_IDX, &cfg);
