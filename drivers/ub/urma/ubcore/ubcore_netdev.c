...
int ubcore_find_port_netdev(struct ubcore_device *dev, struct net_device *ndev)
{
	...
	list_for_each_entry(port_info, &dev->port_list, node) {
		if (port_info->ndev == ndev) {
			...
			return (int)port_info->port_id;
		}
	}
	up_write(&g_port_list_lock);
	ubcore_log_warn("ndev:%s no available port found.\n", netdev_name(ndev));
	/* Currently assigned port0 by default */
	return 0;
}
...
int ubcore_find_port_with_dev_name(struct ubcore_device *dev, char *dev_name)
{
	...
	down_write(&g_port_list_lock);
	list_for_each_entry(port_info, &dev->port_list, node) {
		if (strcmp(port_info->dev_name, dev_name) == 0) {
			up_write(&g_port_list_lock);
			ubcore_log_info("ndev:%s dev name:%s with port id %u found.\n",
				netdev_name(port_info->ndev), dev_name, port_info->port_id);
			return (int)port_info->port_id;
		}
	}
	up_write(&g_port_list_lock);
	ubcore_log_warn("dev name:%s no available port found.\n", dev_name);
	return 0;
}
	...
	unsigned int port_id)
{
	...
	list_for_each_entry(port_info, &dev->port_list, node) {
		if (port_info->ndev == ndev) {
			up_write(&g_port_list_lock);
			ubcore_log_warn("ndev:%s is already bound port: %d\n",
				netdev_name(ndev), port_info->port_id);
			return 0;
		}
	}
	up_write(&g_port_list_lock);
	...
	up_write(&g_port_list_lock);
	ubcore_log_info("ndev:%s dev_name: %s bound port: %d\n", netdev_name(ndev),
		dev->dev_name, new_node->port_id);
	return 0;
}
