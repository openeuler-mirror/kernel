...
{
	u64 dec_nano_mode = 0;
	u8 dec_type = fourcc_mod_vs_get_type(modifier);

	dec_config->dec_mod = DEC_MODE_DISABLE;

	if (dec_type == DRM_FORMAT_MOD_VS_TYPE_ETC2)
		dec_config->dec_mod = DEC_MODE_ETC2;
	else if (dec_type == DRM_FORMAT_MOD_VS_TYPE_DECNANO) {
		dec_nano_mode = fourcc_mod_vs_get_dec_nano_mode(modifier);
		if (dec_nano_mode == DRM_FORMAT_MOD_VS_DECNANO_H_SAMPLE)
			dec_config->dec_mod = DEC_MODE_DEC_NANO_HSAMPLE;
		else if (dec_nano_mode == DRM_FORMAT_MOD_VS_DECNANO_HV_SAMPLE)
...
static void update_fb_modifier_stride(struct drm_framebuffer *drm_fb)
{
	...
	if (dec_type == DRM_FORMAT_MOD_VS_TYPE_DECNANO) {
		switch (tile_mod) {
		case DRM_FORMAT_MOD_VS_DEC_TILE_4X4:
			switch (format) {
			case DRM_FORMAT_ARGB8888:
			case DRM_FORMAT_RGB888:
				norm_mode = DRM_FORMAT_MOD_VS_TILE_MODE4X4;
				break;
			default:
				break;
			}
		...
		case DRM_FORMAT_MOD_VS_DEC_LINEAR:
			...
			case DRM_FORMAT_ARGB8888:
			case DRM_FORMAT_RGB888:
				norm_mode = DRM_FORMAT_MOD_VS_LINEAR;
				break;
			default:
				break;
			}
	...
	} else if (dec_type == DRM_FORMAT_MOD_VS_TYPE_ETC2) {
		if (tile_mod == DRM_FORMAT_MOD_VS_DEC_TILE_4X4 && format == DRM_FORMAT_ARGB8888) {
			norm_mode = DRM_FORMAT_MOD_VS_TILE_MODE4X4;
			drm_fb->pitches[0] = drm_fb->pitches[0] / 4;
		}
	}
	drm_fb->modifier = fourcc_mod_vs_norm_code(norm_mode);
}
...
int dc_dec_config(struct dc_dec *dec_config, struct drm_framebuffer *drm_fb)
{
	...
	if (!drm_fb || ((dec_type != DRM_FORMAT_MOD_VS_TYPE_DECNANO) &&
...
{
	if (dec_config->dirty) {
		dc_hw_set_plane_fbc_dec(hw, id, dec_config->enable, dec_config->dec_mod);
