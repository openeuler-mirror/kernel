 ...
 *
 * Modified: 2026-03-30
 *   - Removed typedef, replaced with struct/enum for checkpatch.pl compliance
 */
...
static u32 _address_to_index(struct _dc_mmu *mmu, u32 address)
{
...
static int _allocate_stlb(struct _dc_mmu_stlb **stlb)
{
	struct _dc_mmu_stlb *stlb_t = NULL;
	void *mem = NULL;
	...
	stlb_t = (struct _dc_mmu_stlb *)mem;
...
static int _allocate_all_stlb(struct device *dev, struct _dc_mmu_stlb **stlb)
{
	struct _dc_mmu_stlb *stlb_t = NULL;
	void *mem = NULL;
	void *cookie = NULL;
...
static int _setup_process_address_space(struct device *dev, struct _dc_mmu *mmu)
{
	u32 *map = NULL;
	u32 free, i;
	u32 dynamic_mapping_entries, address;
	struct _dc_mmu_stlb *all_stlb;
	int ret = 0;
	...
	for (i = 0; i < dynamic_mapping_entries; i++) {
		struct _dc_mmu_stlb *stlb;
		struct _dc_mmu_stlb **stlbs = (struct _dc_mmu_stlb **)mmu->stlbs;
...
int egt_dc_mmu_construct(struct device *dev, struct _dc_mmu **mmu)
{
	...
	size = MMU_MTLB_ENTRY_NUM * sizeof(struct _dc_mmu_stlb);
	...
	memset(mmu_t->safe_page_logical, 0, mmu_t->safe_page_size);
...
static int dc_mmu_get_page_entry(struct _dc_mmu *mmu, u32 address, u32 **page_table)
{
	struct _dc_mmu_stlb *stlb;
	struct _dc_mmu_stlb **stlbs = (struct _dc_mmu_stlb **)mmu->stlbs;
...
static int _link(struct _dc_mmu *mmu, u32 index, u32 node)
{
...
static int _add_free(struct _dc_mmu *mmu, u32 index, u32 node, u32 count)
{
	u32 *map = mmu->map_logical;
...
/* Collect free nodes */
static int _collect(struct _dc_mmu *mmu)
{
	u32 *map = mmu->map_logical;
...
static int dc_mmu_allocate_pages(struct _dc_mmu *mmu, u32 page_count, u32 *address)
{
...
static int dc_mmu_free_pages(struct _dc_mmu *mmu, u32 address, u32 page_count)
{
	u32 *node;
...
static int dc_mmu_set_page(struct _dc_mmu *mmu, u64 page_address, u32 *page_entry)
{
	u32 address_ext;
	u32 address;
