	...
	for (i = 0; i < properties->num; i++) {
		if (property == properties->items[i].data) {
			...
			case VS_DC_PROPERTY_BLOB:
				...
				return ret;
			case VS_DC_PROPERTY_ARRAY:
				type_size = proto->sub_proto.array.dynamic ? 0 :
								proto->type_size;
					   ...
					   const void *obj_state)
{
	...
	for (i = 0; i < registered_drm_properties_num; i++) {
		const struct vs_drm_property_state *state = &drm_states[i];
		struct vs_dc_property_state *hw_state;
		const void *new_data = NULL;
		const struct vs_dc_property_proto *proto = dc_states->items[i].proto;
		u32 size = 0;
...
static const void *__get_drm_property(const struct vs_drm_property_state *states, u32 num,
