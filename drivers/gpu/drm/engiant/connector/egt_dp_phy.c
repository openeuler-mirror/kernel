...
void egt_dp_set_phy(struct egt_displayport *dp, u8 bw_code)
{
	...
	if (timeout <= 0)
		pr_err("egt phy pll lock timeout\n");

	egt_dp_after_phy_locked(dp, bw_code);
}

static void egt_dp_pixel_pll_cfg(struct egt_displayport *dp, struct egt_dc_pll_config *config)
{
	u32 pll_cfg1 = 0;
	u32 pll_cfg2 = 0;
	u32 pll_cfg3 = 0;
	...
	if (timeout <= 0)
		pr_err("pixel pll lock timeout\n");
}

static int egt_dp_try_pll_config(struct egt_dc_pll_config *config, struct egt_displayport *dp,
			uint8_t div_clk, uint8_t N0, u32 M, u64 expect_pll_out2)
{
	...
	for (N2 = 0; N2 <= 15; N2++) {
		for (N1 = 0; N1 <= 15; N1++) {
			...
			vco_khz = (REF_KHZ * vco_component) / N0 / F_SCALE;
			pll_out1_khz = vco_khz / (N1 + 1);
			auto_pll_out2 = pll_out1_khz / (N2 + 1);
			if (auto_pll_out2 == expect_pll_out2 && (vco_khz >= 950000 && vco_khz <= 3800000)) {
				config->N0_cfg = N0;
				config->M_cfg = M;
				config->F_cfg = F;
				config->N1_cfg = N1;
...
static void egt_dp_calculate_pll_config(int freq_khz, struct egt_dc_pll_config *config, struct egt_displayport *dp)
{
	u8 div_clk = 0;
	u8 N0 = 0;
	u32 M = 0;
	...
	for (div_clk = 1; div_clk <= 127; div_clk++) {
		expect_pll_out2 = (freq_khz / 4) * div_clk;
		for (N0 = 1; N0 <= 127; N0++) {
			for (M = 40; M <= 4095; M++) {
				if (egt_dp_try_pll_config(config, dp, div_clk, N0, M, expect_pll_out2))
					return;
			}
		}
	}
