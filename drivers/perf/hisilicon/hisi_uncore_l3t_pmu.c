...
static int hisi_l3t_pmu_probe(struct platform_device *pdev)
{
	...
	ret = cpuhp_state_add_instance(CPUHP_AP_PERF_ARM_HISI_L3T_ONLINE,
				       &l3t_pmu->node);
	if (ret) {
	...
	if (ret) {
		...
		cpuhp_state_remove_instance(CPUHP_AP_PERF_ARM_HISI_L3T_ONLINE,
					    &l3t_pmu->node);
	}
...
static int __init hisi_l3t_pmu_module_init(void)
{
	...
	ret = cpuhp_setup_state_multi(CPUHP_AP_PERF_ARM_HISI_L3T_ONLINE,
				      "AP_PERF_ARM_HISI_L3T_ONLINE",
				      hisi_uncore_pmu_online_cpu,
				      hisi_uncore_pmu_offline_cpu);
	if (ret) {
		pr_err("L3T PMU: Error setup hotplug, ret = %d\n", ret);
		return ret;
	}

	ret = platform_driver_register(&hisi_l3t_pmu_driver);
	if (ret)
		cpuhp_remove_multi_state(CPUHP_AP_PERF_ARM_HISI_L3T_ONLINE);
