...
struct vfd_ops {
	...
	int (*set_trunk)(struct pci_dev *pdev, int vf_id, const unsigned long *buff);
	int (*get_vlan_mirror)(struct pci_dev *pdev, int vf_id, unsigned long *buff);
	int (*set_vlan_mirror)(struct pci_dev *pdev, int vf_id, const unsigned long *buff);
	...
	int (*set_vlan_anti_spoof)(struct pci_dev *pdev, int vf_id, const bool data);
	...
	int (*get_mac_list)(struct pci_dev *pdev, int vf_id, struct list_head *mac_list);
	int (*add_macs_to_list)(struct pci_dev *pdev, int vf_id, struct list_head *mac_list);
	int (*rem_macs_from_list)(struct pci_dev *pdev, int vf_id, struct list_head *mac_list);
	...
	int (*get_max_tx_rate)(struct pci_dev *pdev, int vf_id, unsigned int *max_tx_rate);
	int (*set_max_tx_rate)(struct pci_dev *pdev, int vf_id, unsigned int *max_tx_rate);
	int (*get_min_tx_rate)(struct kobject *kobj, struct kobj_attribute *attr, char *buf);
	int (*set_min_tx_rate)(struct kobject *kobj,
			      struct kobj_attribute *attr, const char *buf, size_t count);
	int (*get_spoofcheck)(struct kobject *kobj, struct kobj_attribute *attr, char *buf);
	int (*set_spoofcheck)(struct kobject *kobj,
			      struct kobj_attribute *attr, const char *buf, size_t count);
	int (*get_trust)(struct kobject *kobj, struct kobj_attribute *attr, char *buf);
	int (*set_trust)(struct kobject *kobj, struct kobj_attribute *attr,
	...
	int (*get_pf_qos_tc_priority)(struct pci_dev *pdev, int tc, char *tc_bitmap);
	int (*set_pf_qos_tc_priority)(struct pci_dev *pdev, int tc, char tc_bitmap);
	int (*get_vf_qos_tc_share)(struct pci_dev *pdev, int vf_id, int tc, u8 *share);
	int (*set_vf_qos_tc_share)(struct pci_dev *pdev, int vf_id, int tc, u8 share);
	int (*get_vf_max_tc_tx_rate)(struct pci_dev *pdev, int vf_id, int tc, int *rate);
	int (*set_vf_max_tc_tx_rate)(struct pci_dev *pdev, int vf_id, int tc, int rate);
};
