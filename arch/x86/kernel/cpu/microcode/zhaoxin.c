...
static struct microcode_zhaoxin *get_microcode_blob(struct ucode_cpu_info *uci, bool save)
{
	struct cpio_data cp;
...
void __init load_ucode_zhaoxin_bsp(void)
{
	...
	if (uci.mc && apply_microcode_early(&uci) == UCODE_UPDATED) {
		...
		pr_debug("%s: BSP CPU %d early update failed due to microcode application failure\n",
...
int __init save_microcode_in_initrd_zhaoxin(void)
{
	...
	if (uci.mc) {
		...
		if (zhaoxin_ucode_patch)
			return 0;
		pr_err("%s: CPU %d save patch failed due to page allocation failure\n",
		       __func__, smp_processor_id());
	}
