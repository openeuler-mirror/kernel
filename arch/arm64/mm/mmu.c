...
static phys_addr_t
try_pgd_pgtable_alloc_init_mm(void)
{
	void *ptr;

	ptr = (void *)__get_free_page(GFP_PGTABLE_KERNEL & ~__GFP_ZERO);
	if (!ptr)
...
static int split_pmd(pmd_t *pmdp, pmd_t pmd)
{
	...
	pte_phys = try_pgd_pgtable_alloc_init_mm();
...
static int split_pud(pud_t *pudp, pud_t pud)
{
	...
	pmd_phys = try_pgd_pgtable_alloc_init_mm();
