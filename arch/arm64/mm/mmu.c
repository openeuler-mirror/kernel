...
static phys_addr_t
try_pgd_pgtable_alloc_init_mm(gfp_t gfp)
{
	void *ptr;

	ptr = (void *)__get_free_page(gfp & ~__GFP_ZERO);
	if (!ptr)
...
static int split_pmd(pmd_t *pmdp, pmd_t pmd, gfp_t gfp, bool to_cont)
{
	...
	pte_phys = try_pgd_pgtable_alloc_init_mm(gfp);
...
static int split_pud(pud_t *pudp, pud_t pud, gfp_t gfp, bool to_cont)
{
	...
	pmd_phys = try_pgd_pgtable_alloc_init_mm(gfp);
...
void __init init_idmap_kpti_bbml3_flag(void)
{
...
{
	if (linear_map_requires_bbml3 && !cpus_have_cap(ARM64_HAS_BBML3)) {
...
static void __init map_mem(pgd_t *pgdp)
{
	...
	if (force_pte_mapping() || is_virtcca_cvm_world())
		flags |= NO_BLOCK_MAPPINGS | NO_CONT_MAPPINGS;
...
static void __init create_idmap(void)
{
	...
	if (linear_map_requires_bbml3 || IS_ENABLED(CONFIG_UNMAP_KERNEL_AT_EL0)) {
		u64 pa = __pa_symbol(&idmap_kpti_bbml3_flag);
