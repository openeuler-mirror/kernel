...
{
	struct wb_switch_ctx *ctx = container_of(work, struct wb_switch_ctx,
						 switch_work);
	struct bdi_writeback *new_wb = ctx->wb;
	struct inode_switch_wbs_context *isw, *next_isw;
	struct llist_node *list;

	list = llist_del_all(&ctx->switch_wbs_ctxs);
	/*
...
static void wb_queue_isw(struct bdi_writeback *wb,
			 struct inode_switch_wbs_context *isw)
{
	if (llist_add(&isw->list, &wb->switch_ctx->switch_wbs_ctxs))
		queue_work(isw_wq, &wb->switch_ctx->switch_work);
}
