...
#define FS_FILE_READ_MODE_MASK (FMODE_RANDOM | FMODE_WILLNEED | FMODE_SPC_READAHEAD)
...
void fs_file_read_update_args_by_trace(struct kiocb *iocb)
{
	...
	filp->f_mode |= ctx.set_f_mode & FS_FILE_READ_MODE_MASK;
	filp->f_mode &= ~(ctx.clr_f_mode & FS_FILE_READ_MODE_MASK);
}
