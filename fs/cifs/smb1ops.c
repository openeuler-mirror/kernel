	    ...
	    struct TCP_Server_Info *server)
{
	...
	if (data_area_of_src < second_buf +
	    ...
	    second_buf + smbCalcSize((struct smb_hdr *)second_buf, server)) {
		  ...
		  char *buf, int malformed)
{
	...
	if (mid->resp_buf) {
		...
		malformed = coalesce_t2(buf, mid->resp_buf, server);
		if (malformed > 0)
			return true;
...
static unsigned int
cifs_negotiate_wsize(struct cifs_tcon *tcon, unsigned int vol_wsize)
{
...
static unsigned int
cifs_negotiate_rsize(struct cifs_tcon *tcon, unsigned int vol_rsize)
{
	__u64 unix_cap = le64_to_cpu(tcon->fsUnixInfo.Capability);
	struct TCP_Server_Info *server = tcon->ses->server;
	...
	rsize = vol_rsize ? vol_rsize : defsize;
