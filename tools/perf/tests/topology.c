...
static int check_cpu_topology(char *path, struct perf_cpu_map *map)
{
	...
	for (i = 0; i < map->nr; i++) {
		TEST_ASSERT_VAL("Core ID doesn't match",
			(session->header.env.cpu[map->map[i].cpu].core_id == (cpu_map__get_core(map, i, NULL) & 0xffff)));

		TEST_ASSERT_VAL("Socket ID doesn't match",
			(session->header.env.cpu[map->map[i].cpu].socket_id == cpu_map__get_socket(map, i, NULL)));
	}
