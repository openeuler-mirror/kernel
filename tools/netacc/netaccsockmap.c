...
static inline int netacc_strncmp(const char *cs, const char *ct, size_t count)
{
	unsigned char c1, c2;
...
static inline int update_netacc_info(struct bpf_sock_ops *skops)
{
	...
	if (netacc_strncmp(comm, "redis-server", 12))
		return 0;
...
SEC("sockops") int netacc_sockops(struct bpf_sock_ops *skops)
{
	switch (skops->op) {
